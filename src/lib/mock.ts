import {
  MockCustomers,
  MockMonthlyType,
  MockFixedExpense,
  ChartAccountsType,
  MockTransactionsType,
  MockCustomersVariables,
} from "@/lib/types";

export const openingBalance = [
  { date: "2025-08-16T12:00:00", openingBalance: 0.0, bank: "Cora" },
  { date: "2025-08-24T12:00:00", openingBalance: 0.0, bank: "Asaas" }, 
  { date: "2025-08-16T12:00:00", openingBalance: 0.0, bank: "Santander" }
];

export const mockCustomers: MockCustomers[] = [
  {
    "id": "ct-0001",
    "cnpj": "31.189.690/0001-00",
    "companyName": "T. C DE SOUSA & CIA LTDA ou SANTOS E CUNHA LTDA",
    "fantasyName": "Diamond",
    "email": "tiangua@clinicasdiamond.com",
    "phone": "(86) 9413-8179",
    "signedContract": "",
    "signedDate": "",
    "dueDate": "2025-10-20T12:00:00",
    "contractTime": "",
    "contractStatus": "Ativo",
    "monthlyFee": 3000.0,
    "terminationClause": "",
    "address": {
      "street": "",
      "avenue": "Avenida Prefeito Jaques Nunes",
      "number": 1028,
      "neighborhood": "Centro",
      "city": "Tianguá",
      "state": "CE"
    }
  },
  {
    "id": "ct-0002",
    "cnpj": "54.687.930/0001-94",
    "companyName": "FRAGA E REZENDE LTDA",
    "fantasyName": "FarmaFormula",
    "email": "lutrf1@gmail.com",
    "phone": "(88) 9206-8076",
    "signedContract": "",
    "signedDate": "",
    "dueDate": "2025-10-15T12:00:00",
    "contractTime": "",
    "contractStatus": "Ativo",
    "monthlyFee": 2000.0,
    "terminationClause": "",
    "address": {
      "street": "Rua 31 de Julho",
      "avenue": "",
      "number": 200,
      "neighborhood": "Ibiapaba shopping Centro",
      "city": "Tianguá",
      "state": "CE"
    }
  },
  {
    "id": "ct-0003",
    "cnpj": "05.356.440/0001-08",
    "companyName": "J B FERREIRA DE SA",
    "fantasyName": "JLanches",
    "email": "-",
    "phone": "(88) 3671-0000",
    "signedContract": "",
    "signedDate": "",
    "dueDate": "2025-10-20T12:00:00",
    "contractTime": "",
    "contractStatus": "Ativo",
    "monthlyFee": 2500.0,
    "terminationClause": "",
    "address": {
      "street": "",
      "avenue": "Avenida Prefeito Jaques Nunes",
      "number": 2172,
      "neighborhood": "Lions Club",
      "city": "Tianguá",
      "state": "CE"
    }
  },
  {
    "id": "ct-0004",
    "cnpj": "43.213.742/0001-08",
    "companyName": "F OLIVER AGUIAR LOPES LTDA",
    "fantasyName": "Casa de Cecília",
    "email": "franzemlopes@hotmail.com",
    "phone": "(88) 9717-6200",
    "signedContract": "",
    "signedDate": "",
    "dueDate": "2025-10-20T12:00:00",
    "contractTime": "",
    "contractStatus": "Ativo",
    "monthlyFee": 3000.0,
    "terminationClause": "",
    "address": {
      "street": "",
      "avenue": "Avenida Prefeito Jaques Nunes",
      "number": 1048,
      "neighborhood": "Centro",
      "city": "Tianguá",
      "state": "CE"
    }
  },
  {
    "id": "ct-0005",
    "cnpj": "26.158.183/0004-04",
    "companyName": "KAROLYNE SIQUEIRA FREIRE LTDA",
    "fantasyName": "Perolab",
    "email": "otica_perola@hotmail.com",
    "phone": "(88) 9344-7144",
    "signedContract": "",
    "signedDate": "",
    "dueDate": "2025-10-20T12:00:00",
    "contractTime": "",
    "contractStatus": "Suspenso",
    "monthlyFee": 2300.0,
    "terminationClause": "",
    "address": {
      "street": "Rua Maria do Socorro Frota e Fontenele",
      "avenue": "",
      "number": 381,
      "neighborhood": "Regis Diniz",
      "city": "Tianguá",
      "state": "CE"
    }
  },
  {
    "id": "ct-0006",
    "cnpj": "",
    "companyName": "FELIPE FROTA",
    "fantasyName": "",
    "email": "",
    "phone": "",
    "signedContract": "",
    "signedDate": "",
    "dueDate": "2025-10-10T12:00:00",
    "contractTime": "",
    "contractStatus": "",
    "monthlyFee": 3000.0,
    "terminationClause": "",
    "address": {
      "street": "",
      "avenue": "",
      "number": 0,
      "neighborhood": "",
      "city": "",
      "state": ""
    }
  }
];

export const mockExpensesFixed: MockFixedExpense[] = [
  {
    "id": "ep-0001",
    "description": "Processo de alteração contábil",
    "paymentMethod": "Transferencia",
    "amount": 1000.0,
    "dueDate": "2025-10-definirT12:00:00",
    "observation": "Apenas uma vez",
    "type": ""
  },
  {
    "id": "ep-0002",
    "description": "João Vitor",
    "paymentMethod": "Transferencia",
    "amount": 1500.0,
    "dueDate": "2025-10-01T12:00:00",
    "observation": "Carteira assinada",
    "type": "Colaboradores - Salários"
  },
  {
    "id": "ep-0003",
    "description": "Saymon",
    "paymentMethod": "Transferencia",
    "amount": 800.0,
    "dueDate": "2025-10-01T12:00:00",
    "observation": "Estagiário",
    "type": "Colaboradores - Salários"
  },
  {
    "id": "ep-0004",
    "description": "Mayane",
    "paymentMethod": "Transferencia",
    "amount": 1500.0,
    "dueDate": "2025-10-01T12:00:00",
    "observation": "Sócia sem recolhimento",
    "type": "Colaboradores - Salários"
  },
  {
    "id": "ep-0005",
    "description": "Júlio César",
    "paymentMethod": "Transferencia",
    "amount": 4000.0,
    "dueDate": "2025-10-01T12:00:00",
    "observation": "Sócio sem recolhimento",
    "type": "Colaboradores - Salários"
  },
  {
    "id": "ep-0006",
    "description": "Internet",
    "paymentMethod": "Boleto",
    "amount": 92.17,
    "dueDate": "2025-10-10T12:00:00",
    "observation": "",
    "type": "Despesas de funcionamento"
  },
  {
    "id": "ep-0007",
    "description": "Fatura Santander",
    "paymentMethod": "Cartão",
    "amount": 819.23,
    "dueDate": "2025-10-15T12:00:00",
    "observation": "",
    "type": "Cartão de crédito"
  },
  {
    "id": "ep-0008",
    "description": "INSS",
    "paymentMethod": "Guia",
    "amount": 0.0,
    "dueDate": "2025-10-20T12:00:00",
    "observation": "Encargo CLT",
    "type": "Impostos"
  },
  {
    "id": "ep-0009",
    "description": "FGTS",
    "paymentMethod": "Guia",
    "amount": 0.0,
    "dueDate": "2025-10-20T12:00:00",
    "observation": "Encargo CLT",
    "type": "Impostos"
  },
  {
    "id": "ep-0010",
    "description": "Aluguel + Energia",
    "paymentMethod": "Desconto",
    "amount": 700.0,
    "dueDate": "2025-10-20T12:00:00",
    "observation": "",
    "type": "Despesas de funcionamento"
  },
  {
    "id": "ep-0011",
    "description": "Software – Adobe (cartão hype)",
    "paymentMethod": "Cartão",
    "amount": 92.0,
    "dueDate": "2025-10-20T12:00:00",
    "observation": "-",
    "type": "Cartão de crédito"
  },
  {
    "id": "ep-0012",
    "description": "Software – Google",
    "paymentMethod": "Cartão",
    "amount": 105.0,
    "dueDate": "2025-10-20T12:00:00",
    "observation": "-",
    "type": "Cartão de crédito"
  },
  {
    "id": "ep-0013",
    "description": "Tripé",
    "paymentMethod": "Cartão",
    "amount": 49.44,
    "dueDate": "2025-10-20T12:00:00",
    "observation": "18x (2/18) – Finaliza Dez/26",
    "type": "Cartão de crédito"
  },
  {
    "id": "ep-0014",
    "description": "Treinamento Don",
    "paymentMethod": "Cartão",
    "amount": 149.44,
    "dueDate": "2025-10-20T12:00:00",
    "observation": "12x (2/12) – Finaliza Jun/26",
    "type": "Cartão de crédito"
  },
  {
    "id": "ep-0015",
    "description": "DAF/Gaveteiros",
    "paymentMethod": "Cartão",
    "amount": 66.93,
    "dueDate": "2025-10-20T12:00:00",
    "observation": "3x (2/3) – Finaliza Set/25",
    "type": "Cartão de crédito"
  },
  {
    "id": "ep-0016",
    "description": "Ar-condicionado",
    "paymentMethod": "Cartão",
    "amount": 291.83,
    "dueDate": "2025-10-20T12:00:00",
    "observation": "10x (8/10) – Finaliza Out/25",
    "type": "Cartão de crédito"
  },
  {
    "id": "ep-0017",
    "description": "Simples Nacional",
    "paymentMethod": "Guia",
    "amount": 600.0,
    "dueDate": "2025-10-20T12:00:00",
    "observation": "6% do Faturamento (Após alteração)",
    "type": "Impostos"
  },
  {
    "id": "ep-0018",
    "description": "Mensalidade BPO",
    "paymentMethod": "Boleto",
    "amount": 300.0,
    "dueDate": "2025-10-25T12:00:00",
    "observation": "A partir de setembro",
    "type": "Contabilidade e serviços extras"
  },
  {
    "id": "ep-0019",
    "description": "Mensalidade Contabilidade",
    "paymentMethod": "Boleto",
    "amount": 80.0,
    "dueDate": "2025-10-25T12:00:00",
    "observation": "Após alteração",
    "type": "Contabilidade e serviços extras"
  },
  {
    "id": "ep-0020",
    "description": "Seguro",
    "paymentMethod": "-",
    "amount": 0.0,
    "dueDate": "2025-10-28T12:00:00",
    "observation": "Encargo Estagiário",
    "type": "Colaboradores - Encargos"
  },
  {
    "id": "ep-0021",
    "description": "Agência",
    "paymentMethod": "-",
    "amount": 0.0,
    "dueDate": "2025-10-28T12:00:00",
    "observation": "Encargo Estagiário",
    "type": "Colaboradores - Encargos"
  },
  {
    "id": "ep-0022",
    "description": "Provisão 13º",
    "paymentMethod": "-",
    "amount": 0.0,
    "dueDate": "2025-10-28T12:00:00",
    "observation": "Provisão mensal CLT",
    "type": "Colaboradores - Encargos"
  },
  {
    "id": "ep-0023",
    "description": "Provisão Férias",
    "paymentMethod": "-",
    "amount": 0.0,
    "dueDate": "2025-10-28T12:00:00",
    "observation": "Provisão mensal CLT",
    "type": "Colaboradores - Encargos"
  }
];

export const mockCustomersVariables: MockCustomersVariables[] = [
  {
    "id": "ct-0001",
    "date": "2025-09-30T12:00:00",
    "customer": "JANAINA ALBUQUE…",
    "event": "15 anos",
    "total": 3000.0,
    "paid": 3000,
    "open": 0
  },
  {
    "id": "ct-0002",
    "date": "2025-09-19T12:00:00",
    "customer": "DANILA C M CERQU…",
    "event": "15 anos",
    "total": 3000.0,
    "paid": 3000,
    "open": 0
  },
  {
    "id": "ct-0003",
    "date": "2025-09-03T12:00:00",
    "customer": "ANA VLADIA MOREI…",
    "event": "15 anos",
    "total": 2400.0,
    "paid": 2400,
    "open": 0
  },
  {
    "id": "ct-0004",
    "date": "2025-09-15T12:00:00",
    "customer": "JOSE INACIO SOUSA ALBUQUERQUE",
    "event": "Cobertura Evento",
    "total": 1300.0,
    "paid": 650,
    "open": 0
  }
];

function createMockMonthly(data: MockTransactionsType[]) {
  const monthName = [
    { name: 'Jan', number: 0 },
    { name: 'Fev', number: 1 }, 
    { name: 'Mar', number: 2 }, 
    { name: 'Abr', number: 3 },
    { name: 'Mai', number: 4 },
    { name: 'Jun', number: 5 },
    { name: 'Jul', number: 6 },
    { name: 'Ago', number: 7 },
    { name: 'Set', number: 8 }, 
    { name: 'Out', number: 9 },
    { name: 'Nov', number: 10 },
    { name: 'Dez', number: 11 }
  ];

  const result: MockMonthlyType[] = [];

  const dates = data
    .map(item => new Date(item.date))
    .sort((a, b) => a.getTime() - b.getTime());

  const minDate = new Date(dates[0].getFullYear(), dates[0].getMonth(), 1);
  const maxDate = new Date(dates[dates.length - 1].getFullYear(), dates[dates.length - 1].getMonth(), 1);

  const cursor = new Date(minDate);

  while (cursor <= maxDate) {
    const year = cursor.getFullYear();
    const month = cursor.getMonth();

    const filtered = data.filter(register => {
      const d = new Date(register.date);
      return d.getFullYear() === year && d.getMonth() === month;
    });

    const variableRevenue = filtered
      .filter(r => r.typeId === 1 && r.accountId === 2)
      .reduce((acc, cur) => acc + cur.amount, 0);

    const fixedRevenue = filtered
      .filter(r => r.typeId === 1 && r.accountId === 3)
      .reduce((acc, cur) => acc + cur.amount, 0);

    const cost = filtered
      .filter(r => r.typeId === 3)
      .reduce((acc, cur) => acc + cur.amount, 0);

    const expense = filtered
      .filter(r => r.typeId === 2 && r.accountId !== 104)
      .reduce((acc, cur) => acc + cur.amount, 0);

    result.push({
      month: monthName[month].name,
      date: `${year}-${String(month + 1).padStart(2, '0')}-01`,
      fixedRevenue,
      variableRevenue,
      expense,
      cost,
    });

    cursor.setMonth(cursor.getMonth() + 1);
  }

  const averageOrderValueFixed =
    result.reduce((acc, cur) => acc + cur.fixedRevenue, 0) / result.length;

  const averageOrderValueEvent =
    result.reduce((acc, cur) => acc + cur.variableRevenue, 0) / result.length;

  return { month: result, averageOrderValueFixed, averageOrderValueEvent };
}

export const mockTransactions: MockTransactionsType[] = [
  {
    "id": "tx-0001",
    "date": "2025-08-22T12:00:00",
    "description": "Transf Pix recebida",
    "history": "Mensalidade",
    "customerId": "43.213.742/0001-08",
    "customer": "Casa de Cecília",
    "typeId": 1,
    "type": "Credito",
    "amount": 3000.0,
    "accountId": 3,
    "bank": "Cora"
  },
  {
    "id": "tx-0002",
    "date": "2025-08-22T12:00:00",
    "description": "Transf Pix recebida",
    "history": "Mensalidade",
    "customerId": "31.189.690/0001-00",
    "customer": "Diamond",
    "typeId": 1,
    "type": "Credito",
    "amount": 1000.0,
    "accountId": 3,
    "bank": "Cora"
  },
  {
    "id": "tx-0003",
    "date": "2025-08-23T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "Gaveteiros",
    "customerId": "10.573.521/0001-91",
    "customer": "MERCADO PAGO IN…",
    "typeId": 2,
    "type": "Debito",
    "amount": 66.93,
    "accountId": 77,
    "bank": "Cora"
  },
  {
    "id": "tx-0004",
    "date": "2025-08-23T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "Ar Condicionado",
    "customerId": "10.573.521/0001-91",
    "customer": "MERCADO PAGO IN…",
    "typeId": 2,
    "type": "Debito",
    "amount": 291.83,
    "accountId": 73,
    "bank": "Cora"
  },
  {
    "id": "tx-0005",
    "date": "2025-08-23T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "Tripé",
    "customerId": "10.573.521/0001-91",
    "customer": "MERCADO PAGO IN…",
    "typeId": 2,
    "type": "Debito",
    "amount": 49.44,
    "accountId": 73,
    "bank": "Cora"
  },
  {
    "id": "tx-0006",
    "date": "2025-08-23T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "Trieinamento",
    "customerId": "10.573.521/0001-91",
    "customer": "MERCADO PAGO IN…",
    "typeId": 2,
    "type": "Debito",
    "amount": 149.7,
    "accountId": 98,
    "bank": "Cora"
  },
  {
    "id": "tx-0007",
    "date": "2025-08-23T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "Programa de Edição",
    "customerId": "10.573.521/0001-91",
    "customer": "MERCADO PAGO IN…",
    "typeId": 3,
    "type": "Debito",
    "amount": 95.0,
    "accountId": 25,
    "bank": "Cora"
  },
  {
    "id": "tx-0008",
    "date": "2025-08-23T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "Sistema Clickup",
    "customerId": "10.573.521/0001-91",
    "customer": "MERCADO PAGO IN…",
    "typeId": 2,
    "type": "Debito",
    "amount": 27.95,
    "accountId": 90,
    "bank": "Cora"
  },
  {
    "id": "tx-0009",
    "date": "2025-08-23T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "Sistema Clickup",
    "customerId": "10.573.521/0001-91",
    "customer": "MERCADO PAGO IN…",
    "typeId": 2,
    "type": "Debito",
    "amount": 24.47,
    "accountId": 90,
    "bank": "Cora"
  },
  {
    "id": "tx-0010",
    "date": "2025-08-23T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "Sistema Clickup",
    "customerId": "10.573.521/0001-91",
    "customer": "MERCADO PAGO IN…",
    "typeId": 2,
    "type": "Debito",
    "amount": 324.22,
    "accountId": 90,
    "bank": "Cora"
  },
  {
    "id": "tx-0011",
    "date": "2025-08-25T12:00:00",
    "description": "Transf Pix recebida",
    "history": "Transferencia entre contas",
    "customerId": "41.655.312/0001-10",
    "customer": "41.655.312 JULIO C…",
    "typeId": 1,
    "type": "Credito",
    "amount": 2500.0,
    "accountId": 106,
    "bank": "Cora"
  },
  {
    "id": "tx-0012",
    "date": "2025-08-26T12:00:00",
    "description": "Transf Pix recebida",
    "history": "Mensalidade",
    "customerId": "31.189.690/0001-00",
    "customer": "Diamond",
    "typeId": 1,
    "type": "Credito",
    "amount": 1000.0,
    "accountId": 3,
    "bank": "Cora"
  },
  {
    "id": "tx-0013",
    "date": "2025-08-26T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "Lanches para colaboradores",
    "customerId": "005.929.373-05",
    "customer": "FABRICIO RODRIGU…",
    "typeId": 2,
    "type": "Debito",
    "amount": 12.0,
    "accountId": 65,
    "bank": "Cora"
  },
  {
    "id": "tx-0014",
    "date": "2025-08-26T12:00:00",
    "description": "Transf Pix recebida",
    "history": "Transferencia entre contas",
    "customerId": "41.655.312/0001-10",
    "customer": "41.655.312 JULIO C…",
    "typeId": 1,
    "type": "Credito",
    "amount": 2299.0,
    "accountId": 106,
    "bank": "Cora"
  },
  {
    "id": "tx-0015",
    "date": "2025-08-27T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "Coador de café",
    "customerId": "02.268.603/0001-02",
    "customer": "R G MOREIRA SOUZ…",
    "typeId": 2,
    "type": "Debito",
    "amount": 5.69,
    "accountId": 102,
    "bank": "Cora"
  },
  {
    "id": "tx-0016",
    "date": "2025-08-27T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "Copos descartaveis",
    "customerId": "05.526.674/0001-48",
    "customer": "TEIXEIRA SACOLAS",
    "typeId": 2,
    "type": "Debito",
    "amount": 4.9,
    "accountId": 102,
    "bank": "Cora"
  },
  {
    "id": "tx-0017",
    "date": "2025-08-27T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "Lanches para colaboradores",
    "customerId": "02.268.603/0001-02",
    "customer": "R G MOREIRA SOUZ",
    "typeId": 2,
    "type": "Debito",
    "amount": 13.49,
    "accountId": 65,
    "bank": "Cora"
  },
  {
    "id": "tx-0018",
    "date": "2025-08-27T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "Lanches para colaboradores",
    "customerId": "11.889.648/0001-87",
    "customer": "FRANCISCO JOSE D…",
    "typeId": 2,
    "type": "Debito",
    "amount": 6.5,
    "accountId": 65,
    "bank": "Cora"
  },
  {
    "id": "tx-0019",
    "date": "2025-08-27T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Pró-labore",
    "customerId": "067.164.483-14",
    "customer": "JULIO CESAR RODR…",
    "typeId": 3,
    "type": "Debito",
    "amount": 1500.0,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0020",
    "date": "2025-08-28T12:00:00",
    "description": "Transf Pix recebida",
    "history": "Mensalidade",
    "customerId": "26.719.797/0001-75",
    "customer": "SANTOS E CUNHA LTDA (diamond)",
    "typeId": 1,
    "type": "Credito",
    "amount": 1000.0,
    "accountId": 3,
    "bank": "Cora"
  },
  {
    "id": "tx-0021",
    "date": "2025-08-29T12:00:00",
    "description": "Compra no débito",
    "history": "Combustiveis",
    "customerId": "-",
    "customer": "POSTO CATATAU VII",
    "typeId": 2,
    "type": "Debito",
    "amount": 150.0,
    "accountId": 33,
    "bank": "Cora"
  },
  {
    "id": "tx-0022",
    "date": "2025-08-29T12:00:00",
    "description": "Compra no débito",
    "history": "Cabo tipo-c",
    "customerId": "-",
    "customer": "DENNYSCELL UBAJ…",
    "typeId": 2,
    "type": "Debito",
    "amount": 20.0,
    "accountId": 44,
    "bank": "Cora"
  },
  {
    "id": "tx-0023",
    "date": "2025-08-29T12:00:00",
    "description": "Compra no débito",
    "history": "Caixas para entrega final ao cliente",
    "customerId": "-",
    "customer": "AliceArtefatosEm",
    "typeId": 2,
    "type": "Debito",
    "amount": 51.0,
    "accountId": 31,
    "bank": "Cora"
  },
  {
    "id": "tx-0024",
    "date": "2025-08-30T12:00:00",
    "description": "Compra no débito",
    "history": "Impressão de Fotos",
    "customerId": "-",
    "customer": "SMILICOPY GRAFIVA",
    "typeId": 3,
    "type": "Debito",
    "amount": 5.0,
    "accountId": 101,
    "bank": "Cora"
  },
  {
    "id": "tx-0025",
    "date": "2025-08-30T12:00:00",
    "description": "Compra no débito",
    "history": "3 Pen Drives para entrega ao cliente",
    "customerId": "-",
    "customer": "AMERICANAS SA",
    "typeId": 3,
    "type": "Debito",
    "amount": 89.97,
    "accountId": 101,
    "bank": "Cora"
  },
  {
    "id": "tx-0026",
    "date": "2025-08-30T12:00:00",
    "description": "Compra no débito",
    "history": "Chocolate para entrega final ao cliente",
    "customerId": "-",
    "customer": "CACAU SHOW",
    "typeId": 2,
    "type": "Debito",
    "amount": 15.99,
    "accountId": 31,
    "bank": "Cora"
  },
  {
    "id": "tx-0027",
    "date": "2025-08-30T12:00:00",
    "description": "Compra no débito",
    "history": "Porta-retratos para entrega ao cliente",
    "customerId": "-",
    "customer": "MP*DAFFCE",
    "typeId": 2,
    "type": "Debito",
    "amount": 56.4,
    "accountId": 31,
    "bank": "Cora"
  },
  {
    "id": "tx-0028",
    "date": "2025-09-01T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Pró-labore",
    "customerId": "067.164.483-14",
    "customer": "JULIO CESAR RODR…",
    "typeId": 3,
    "type": "Debito",
    "amount": 2793.06,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0029",
    "date": "2025-09-01T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Salários",
    "customerId": "082.939.493-17",
    "customer": "JOAO VITOR FONT…",
    "typeId": 3,
    "type": "Debito",
    "amount": 1650.0,
    "accountId": 18,
    "bank": "Cora"
  },
  {
    "id": "tx-0030",
    "date": "2025-09-01T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Salários",
    "customerId": "603.804.323-31",
    "customer": "Mayane Lima dos Sa…",
    "typeId": 3,
    "type": "Debito",
    "amount": 1800.0,
    "accountId": 18,
    "bank": "Cora"
  },
  {
    "id": "tx-0031",
    "date": "2025-09-01T12:00:00",
    "description": "Compra no débito",
    "history": "Lanches para colaboradores",
    "customerId": "-",
    "customer": "LIPI SUPERMERCADO",
    "typeId": 2,
    "type": "Debito",
    "amount": 4.69,
    "accountId": 65,
    "bank": "Cora"
  },
  {
    "id": "tx-0032",
    "date": "2025-09-01T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "MEI atrasado",
    "customerId": "00.394.460/0058-87",
    "customer": "MINISTERIO DA FAZ…",
    "typeId": 2,
    "type": "Debito",
    "amount": 84.92,
    "accountId": 89,
    "bank": "Cora"
  },
  {
    "id": "tx-0033",
    "date": "2025-09-01T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Pró-labore",
    "customerId": "067.164.483-14",
    "customer": "JULIO CESAR RODR…",
    "typeId": 3,
    "type": "Debito",
    "amount": 50.0,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0034",
    "date": "2025-09-01T12:00:00",
    "description": "Transf Pix recebida",
    "history": "Esporádico",
    "customerId": "063.536.963-02",
    "customer": "JANAINA ALBUQUE…",
    "typeId": 1,
    "type": "Credito",
    "amount": 3000.0,
    "accountId": 2,
    "bank": "Cora"
  },
  {
    "id": "tx-0035",
    "date": "2025-09-01T12:00:00",
    "description": "Transf Pix recebida",
    "history": "Esporádico",
    "customerId": "992.561.775-87",
    "customer": "DANILA C M CERQU…",
    "typeId": 1,
    "type": "Credito",
    "amount": 2000.0,
    "accountId": 2,
    "bank": "Cora"
  },
  {
    "id": "tx-0036",
    "date": "2025-09-02T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Salários",
    "customerId": "083.039.283-17",
    "customer": "Saimon Nascimento …",
    "typeId": 3,
    "type": "Debito",
    "amount": 950.0,
    "accountId": 16,
    "bank": "Cora"
  },
  {
    "id": "tx-0037",
    "date": "2025-09-02T12:00:00",
    "description": "Transf Pix recebida",
    "history": "Esporádico",
    "customerId": "638.844.613-20",
    "customer": "ANA VLADIA MOREI…",
    "typeId": 1,
    "type": "Credito",
    "amount": 2400.0,
    "accountId": 2,
    "bank": "Cora"
  },
  {
    "id": "tx-0038",
    "date": "2025-09-02T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "MEI mês atual",
    "customerId": "00.394.460/0058-87",
    "customer": "MINISTERIO DA FAZ…",
    "typeId": 2,
    "type": "Debito",
    "amount": 80.9,
    "accountId": 89,
    "bank": "Cora"
  },
  {
    "id": "tx-0039",
    "date": "2025-09-03T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Locação de câmera",
    "customerId": "52.640.748/0001-52",
    "customer": "52.640.748 ANDRE …",
    "typeId": 2,
    "type": "Debito",
    "amount": 180.0,
    "accountId": 88,
    "bank": "Cora"
  },
  {
    "id": "tx-0040",
    "date": "2025-09-05T12:00:00",
    "description": "Compra no débito",
    "history": "Lanches para colaboradores",
    "customerId": "-",
    "customer": "MERCADINHO BOM…",
    "typeId": 2,
    "type": "Debito",
    "amount": 5.58,
    "accountId": 65,
    "bank": "Cora"
  },
  {
    "id": "tx-0041",
    "date": "2025-09-07T12:00:00",
    "description": "Compra no débito",
    "history": "Combustiveis",
    "customerId": "-",
    "customer": "POSTO CATATAU II",
    "typeId": 2,
    "type": "Debito",
    "amount": 100.0,
    "accountId": 33,
    "bank": "Cora"
  },
  {
    "id": "tx-0042",
    "date": "2025-09-08T12:00:00",
    "description": "Compra no débito",
    "history": "Pró-labore",
    "customerId": "-",
    "customer": "MP*CHEFDOACAI",
    "typeId": 3,
    "type": "Debito",
    "amount": 13.12,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0043",
    "date": "2025-09-08T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Eventos",
    "customerId": "082.939.493-17",
    "customer": "JOAO VITOR FONT…",
    "typeId": 3,
    "type": "Debito",
    "amount": 150.0,
    "accountId": 18,
    "bank": "Cora"
  },
  {
    "id": "tx-0044",
    "date": "2025-09-08T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Eventos",
    "customerId": "082.939.493-17",
    "customer": "Saimon Nascimento …",
    "typeId": 3,
    "type": "Debito",
    "amount": 150.0,
    "accountId": 16,
    "bank": "Cora"
  },
  {
    "id": "tx-0045",
    "date": "2025-09-08T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Lanches para colaboradores",
    "customerId": "834.079.953-34",
    "customer": "ANA CRISTINA PINT…",
    "typeId": 2,
    "type": "Debito",
    "amount": 8.0,
    "accountId": 65,
    "bank": "Cora"
  },
  {
    "id": "tx-0046",
    "date": "2025-09-10T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Transferencia entre contas",
    "customerId": "41.655.312/0001-10",
    "customer": "41 655 312 JULIO C…",
    "typeId": 2,
    "type": "Debito",
    "amount": 30.0,
    "accountId": 104,
    "bank": "Cora"
  },
  {
    "id": "tx-0047",
    "date": "2025-09-10T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Transferencia entre contas",
    "customerId": "41.655.312/0001-10",
    "customer": "41 655 312 JULIO C…",
    "typeId": 2,
    "type": "Debito",
    "amount": 30.0,
    "accountId": 104,
    "bank": "Cora"
  },
  {
    "id": "tx-0048",
    "date": "2025-09-10T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Transferencia entre contas",
    "customerId": "41.655.312/0001-10",
    "customer": "41 655 312 JULIO C…",
    "typeId": 2,
    "type": "Debito",
    "amount": 30.0,
    "accountId": 104,
    "bank": "Cora"
  },
  {
    "id": "tx-0049",
    "date": "2025-09-11T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Transferencia entre contas",
    "customerId": "41.655.312/0001-10",
    "customer": "41 655 312 JULIO C…",
    "typeId": 2,
    "type": "Debito",
    "amount": 7129.65,
    "accountId": 104,
    "bank": "Cora"
  },
  {
    "id": "tx-0050",
    "date": "2025-12-15T12:00:00",
    "description": "Transf Pix recebida",
    "history": "Transferencia entre contas",
    "customerId": "41.655.312/0001-10",
    "customer": "41 655 312 JULIO C…",
    "typeId": 1,
    "type": "Credito",
    "amount": 5501.83,
    "accountId": 106,
    "bank": "Cora"
  },
  {
    "id": "tx-0051",
    "date": "2025-12-16T12:00:00",
    "description": "Transf Pix recebida",
    "history": "Transferencia entre contas",
    "customerId": "41.655.312/0001-10",
    "customer": "41 655 312 JULIO C…",
    "typeId": 1,
    "type": "Credito",
    "amount": 300.0,
    "accountId": 106,
    "bank": "Cora"
  },
  {
    "id": "tx-0052",
    "date": "2025-12-16T12:00:00",
    "description": "Transf Pix recebida",
    "history": "Transferencia entre contas",
    "customerId": "41.655.312/0001-10",
    "customer": "41 655 312 JULIO C…",
    "typeId": 1,
    "type": "Credito",
    "amount": 1997.06,
    "accountId": 106,
    "bank": "Cora"
  },
  {
    "id": "tx-0053",
    "date": "2025-12-16T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Pró-labore",
    "customerId": "067.164.483-14",
    "customer": "JULIO CESAR RODR…",
    "typeId": 3,
    "type": "Debito",
    "amount": 400.0,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0054",
    "date": "2025-12-18T12:00:00",
    "description": "Compra no débito",
    "history": "Lanches",
    "customerId": "-",
    "customer": "R G MOREIRA SOUZ…",
    "typeId": 2,
    "type": "Debito",
    "amount": 8.88,
    "accountId": 102,
    "bank": "Cora"
  },
  {
    "id": "tx-0055",
    "date": "2025-12-19T12:00:00",
    "description": "Compra no débito",
    "history": "Despesas com entrega",
    "customerId": "-",
    "customer": "AliceArtefatosEm",
    "typeId": 2,
    "type": "Debito",
    "amount": 15.0,
    "accountId": 31,
    "bank": "Cora"
  },
  {
    "id": "tx-0056",
    "date": "2025-12-20T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Pró-labore",
    "customerId": "067.164.483-14",
    "customer": "JULIO CESAR RODR…",
    "typeId": 3,
    "type": "Debito",
    "amount": 200.0,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0057",
    "date": "2025-12-22T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Pró-labore",
    "customerId": "067.164.483-14",
    "customer": "JULIO CESAR RODR…",
    "typeId": 3,
    "type": "Debito",
    "amount": 80.0,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0058",
    "date": "2025-12-23T12:00:00",
    "description": "Compra no débito",
    "history": "Brindes de Natal",
    "customerId": "-",
    "customer": "UTILMIX",
    "typeId": 2,
    "type": "Debito",
    "amount": 35.3,
    "accountId": 13,
    "bank": "Cora"
  },
  {
    "id": "tx-0059",
    "date": "2025-12-23T12:00:00",
    "description": "Compra no débito",
    "history": "Brindes de Natal",
    "customerId": "-",
    "customer": "MP*FACAFESTA",
    "typeId": 2,
    "type": "Debito",
    "amount": 34.5,
    "accountId": 13,
    "bank": "Cora"
  },
  {
    "id": "tx-0060",
    "date": "2025-12-23T12:00:00",
    "description": "Compra no débito",
    "history": "Brindes de Natal",
    "customerId": "-",
    "customer": "LASA",
    "typeId": 2,
    "type": "Debito",
    "amount": 10.99,
    "accountId": 13,
    "bank": "Cora"
  },
  {
    "id": "tx-0061",
    "date": "2025-12-23T12:00:00",
    "description": "Compra no débito",
    "history": "Brindes de Natal",
    "customerId": "-",
    "customer": "SUPERMERCADO PI…",
    "typeId": 2,
    "type": "Debito",
    "amount": 46.8,
    "accountId": 13,
    "bank": "Cora"
  },
  {
    "id": "tx-0062",
    "date": "2025-12-23T12:00:00",
    "description": "Compra no débito",
    "history": "Brindes de Natal",
    "customerId": "-",
    "customer": "XIMENES ATACAREJO",
    "typeId": 2,
    "type": "Debito",
    "amount": 70.97,
    "accountId": 13,
    "bank": "Cora"
  },
  {
    "id": "tx-0063",
    "date": "2025-12-23T12:00:00",
    "description": "Compra no débito",
    "history": "Brindes de Natal",
    "customerId": "-",
    "customer": "TECIDOS O ZE ARTE",
    "typeId": 2,
    "type": "Debito",
    "amount": 7.5,
    "accountId": 13,
    "bank": "Cora"
  },
  {
    "id": "tx-0064",
    "date": "2025-12-23T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "Brindes de Natal",
    "customerId": "22.706.963/0001-48",
    "customer": "DEBORA VIEIRA G D…",
    "typeId": 2,
    "type": "Debito",
    "amount": 17.5,
    "accountId": 13,
    "bank": "Cora"
  },
  {
    "id": "tx-0065",
    "date": "2025-12-23T12:00:00",
    "description": "Compra no débito",
    "history": "Brindes de Natal",
    "customerId": "-",
    "customer": "TECIDOS O ZE ARTE",
    "typeId": 2,
    "type": "Debito",
    "amount": 7.0,
    "accountId": 13,
    "bank": "Cora"
  },
  {
    "id": "tx-0066",
    "date": "2025-12-24T12:00:00",
    "description": "Compra no débito",
    "history": "Combustível",
    "customerId": "-",
    "customer": "POSTO CATATAU VII",
    "typeId": 2,
    "type": "Debito",
    "amount": 100.0,
    "accountId": 33,
    "bank": "Cora"
  },
  {
    "id": "tx-0067",
    "date": "2025-12-24T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Pró-labore",
    "customerId": "067.164.483-14",
    "customer": "JULIO CESAR RODR…",
    "typeId": 3,
    "type": "Debito",
    "amount": 100.0,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0068",
    "date": "2025-12-26T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "Lanches para colaboradores",
    "customerId": "02.268.603/0001-02",
    "customer": "R G MOREIRA SOUZ…",
    "typeId": 2,
    "type": "Debito",
    "amount": 8.5,
    "accountId": 65,
    "bank": "Cora"
  },
  {
    "id": "tx-0069",
    "date": "2025-12-27T12:00:00",
    "description": "Compra no débito",
    "history": "Pró-labore",
    "customerId": "-",
    "customer": "JIM.COM* MERCAD…",
    "typeId": 3,
    "type": "Debito",
    "amount": 4.0,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0070",
    "date": "2025-12-29T12:00:00",
    "description": "Transf Pix recebida",
    "history": "Transferencia entre contas",
    "customerId": "41.655.312/0001-10",
    "customer": "41.655.312 JULIO C…",
    "typeId": 1,
    "type": "Credito",
    "amount": 2497.02,
    "accountId": 106,
    "bank": "Cora"
  },
  {
    "id": "tx-0071",
    "date": "2025-12-29T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Pró-labore",
    "customerId": "067.164.483-14",
    "customer": "JULIO CESAR RODR…",
    "typeId": 3,
    "type": "Debito",
    "amount": 1861.0,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0072",
    "date": "2026-01-03T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Pró-labore",
    "customerId": "067.164.483-14",
    "customer": "JULIO CESAR RODR…",
    "typeId": 3,
    "type": "Debito",
    "amount": 100.0,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0073",
    "date": "2026-01-04T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Pró-labore",
    "customerId": "067.164.483-14",
    "customer": "JULIO CESAR RODR…",
    "typeId": 3,
    "type": "Debito",
    "amount": 50.0,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0074",
    "date": "2026-01-05T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "Lanches para colaboradores",
    "customerId": "135.179.533-34",
    "customer": "JOAO LOPES MARTI…",
    "typeId": 2,
    "type": "Debito",
    "amount": 13.6,
    "accountId": 65,
    "bank": "Cora"
  },
  {
    "id": "tx-0075",
    "date": "2026-01-06T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Salários",
    "customerId": "082.939.493-17",
    "customer": "JOAO VITOR FONT…",
    "typeId": 3,
    "type": "Debito",
    "amount": 1844.0,
    "accountId": 18,
    "bank": "Cora"
  },
  {
    "id": "tx-0076",
    "date": "2026-01-06T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Pró-labore",
    "customerId": "067.164.483-14",
    "customer": "JULIO CESAR RODR…",
    "typeId": 3,
    "type": "Debito",
    "amount": 100.0,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0077",
    "date": "2026-01-06T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "Uniformes Colaboradores",
    "customerId": "025.029.753-10",
    "customer": "RAFAEL GONDIM B…",
    "typeId": 2,
    "type": "Debito",
    "amount": 40.0,
    "accountId": 99,
    "bank": "Cora"
  },
  {
    "id": "tx-0078",
    "date": "2026-01-06T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Pró-labore",
    "customerId": "067.164.483-14",
    "customer": "JULIO CESAR RODR…",
    "typeId": 3,
    "type": "Debito",
    "amount": 1000.0,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0079",
    "date": "2026-01-09T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "Combustível",
    "customerId": "13.070.068/0002-70",
    "customer": "CATATAU COMERCI…",
    "typeId": 2,
    "type": "Debito",
    "amount": 100.0,
    "accountId": 33,
    "bank": "Cora"
  },
  {
    "id": "tx-0080",
    "date": "2026-01-13T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Lanches para colaboradores",
    "customerId": "18.732.807/0001-84",
    "customer": "RONILDO CASSIO D…",
    "typeId": 2,
    "type": "Debito",
    "amount": 15.0,
    "accountId": 65,
    "bank": "Cora"
  },
  {
    "id": "tx-0081",
    "date": "2026-01-14T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "MEI atrasado",
    "customerId": "00.394.460/0058-87",
    "customer": "MINISTERIO DA FAZ…",
    "typeId": 2,
    "type": "Debito",
    "amount": 80.9,
    "accountId": 89,
    "bank": "Cora"
  },
  {
    "id": "tx-0082",
    "date": "2026-01-14T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "Internet",
    "customerId": "04.601.397/0001-28",
    "customer": "BRISANET SERVICO…",
    "typeId": 2,
    "type": "Debito",
    "amount": 86.76,
    "accountId": 57,
    "bank": "Cora"
  },
  {
    "id": "tx-0083",
    "date": "2026-01-14T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "Internet",
    "customerId": "04.601.397/0001-28",
    "customer": "BRISANET SERVICO…",
    "typeId": 2,
    "type": "Debito",
    "amount": 91.98,
    "accountId": 57,
    "bank": "Cora"
  },
  {
    "id": "tx-0084",
    "date": "2026-01-16T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Pro-labore",
    "customerId": "067.164.483-14",
    "customer": "JULIO CESAR RODR…",
    "typeId": 3,
    "type": "Debito",
    "amount": 100.0,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0085",
    "date": "2026-01-17T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Pro-labore",
    "customerId": "067.164.483-14",
    "customer": "JULIO CESAR RODR…",
    "typeId": 3,
    "type": "Debito",
    "amount": 100.0,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0086",
    "date": "2026-01-18T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Pro-labore",
    "customerId": "067.164.483-14",
    "customer": "JULIO CESAR RODR…",
    "typeId": 3,
    "type": "Debito",
    "amount": 100.0,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0087",
    "date": "2026-01-19T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Pro-labore",
    "customerId": "067.164.483-14",
    "customer": "JULIO CESAR RODR…",
    "typeId": 3,
    "type": "Debito",
    "amount": 1000.0,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0088",
    "date": "2026-01-19T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Salários",
    "customerId": "603.804.323-31",
    "customer": "Mayane Lima dos Sa…",
    "typeId": 3,
    "type": "Debito",
    "amount": 1200.0,
    "accountId": 18,
    "bank": "Cora"
  },
  {
    "id": "tx-0089",
    "date": "2026-01-19T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Transferencia entre contas",
    "customerId": "41.655.312/0001-10",
    "customer": "41.655.312 JULIO C…",
    "typeId": 1,
    "type": "Credito",
    "amount": 1997.02,
    "accountId": 106,
    "bank": "Cora"
  },
  {
    "id": "tx-0090",
    "date": "2026-01-20T12:00:00",
    "description": "Compra no débito",
    "history": "Combustível",
    "customerId": "-",
    "customer": "POSTO CATATAU VII",
    "typeId": 2,
    "type": "Debito",
    "amount": 100.0,
    "accountId": 33,
    "bank": "Cora"
  },
  {
    "id": "tx-0091",
    "date": "2026-01-21T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Transferencia entre contas",
    "customerId": "41.655.312/0001-10",
    "customer": "41 655 312 JULIO C…",
    "typeId": 2,
    "type": "Debito",
    "amount": 960.0,
    "accountId": 104,
    "bank": "Cora"
  },
  {
    "id": "tx-0092",
    "date": "2026-01-21T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "",
    "customerId": "10.573.521/0001-91",
    "customer": "MERCADO PAGO IN…",
    "typeId": 2,
    "type": "Debito",
    "amount": 494.14,
    "accountId": 33,
    "bank": "Cora"
  },
  {
    "id": "tx-0093",
    "date": "2026-01-21T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Pro-labore",
    "customerId": "067.164.483-14",
    "customer": "JULIO CESAR RODR…",
    "typeId": 3,
    "type": "Debito",
    "amount": 200.0,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0094",
    "date": "2026-01-22T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Pro-labore",
    "customerId": "067.164.483-14",
    "customer": "JULIO CESAR RODR…",
    "typeId": 3,
    "type": "Debito",
    "amount": 100.0,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0095",
    "date": "2026-01-23T12:00:00",
    "description": "Compra no débito",
    "history": "Lanches para colaboradores",
    "customerId": "-",
    "customer": "MERCADINHO BOM…",
    "typeId": 2,
    "type": "Debito",
    "amount": 9.0,
    "accountId": 65,
    "bank": "Cora"
  },
  {
    "id": "tx-0096",
    "date": "2026-01-23T12:00:00",
    "description": "Transf Pix recebida",
    "history": "Transferencia entre contas",
    "customerId": "41.655.312/0001-10",
    "customer": "41.655.312 JULIO C…",
    "typeId": 1,
    "type": "Credito",
    "amount": 2497.0,
    "accountId": 106,
    "bank": "Cora"
  },
  {
    "id": "tx-0097",
    "date": "2026-01-26T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Sistemas Contratados",
    "customerId": "027.682.842-97",
    "customer": "Christian Silva Pessoa…",
    "typeId": 2,
    "type": "Debito",
    "amount": 127.5,
    "accountId": 90,
    "bank": "Cora"
  },
  {
    "id": "tx-0098",
    "date": "2026-01-26T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "Carregador PowerBank",
    "customerId": "15.436.940/0001-03",
    "customer": "AMAZON SERVICOS…",
    "typeId": 2,
    "type": "Debito",
    "amount": 158.98,
    "accountId": 73,
    "bank": "Cora"
  },
  {
    "id": "tx-0099",
    "date": "2026-01-29T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Pro-labore",
    "customerId": "067.164.483-14",
    "customer": "JULIO CESAR RODR…",
    "typeId": 3,
    "type": "Debito",
    "amount": 1400.0,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0100",
    "date": "2026-01-31T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Pro-labore",
    "customerId": "067.164.483-14",
    "customer": "JULIO CESAR RODR…",
    "typeId": 3,
    "type": "Debito",
    "amount": 200.0,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0101",
    "date": "2026-02-02T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Salários",
    "customerId": "082.939.493-17",
    "customer": "JOAO VITOR FONT…",
    "typeId": 3,
    "type": "Debito",
    "amount": 1545.0,
    "accountId": 18,
    "bank": "Cora"
  },
  {
    "id": "tx-0102",
    "date": "2026-02-02T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Pro-labore",
    "customerId": "067.164.483-14",
    "customer": "JULIO CESAR RODR…",
    "typeId": 3,
    "type": "Debito",
    "amount": 50.0,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0103",
    "date": "2026-02-02T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "Combustível",
    "customerId": "13.070.068/0002-70",
    "customer": "CATATAU COMERCI…",
    "typeId": 2,
    "type": "Debito",
    "amount": 100.0,
    "accountId": 33,
    "bank": "Cora"
  },
  {
    "id": "tx-0104",
    "date": "2026-02-04T12:00:00",
    "description": "Transf Pix recebida",
    "history": "Projetos Avulsos",
    "customerId": "41.904.774/0001-24",
    "customer": "HUST COSMETICOS…",
    "typeId": 1,
    "type": "Credito",
    "amount": 975.0,
    "accountId": 2,
    "bank": "Cora"
  },
  {
    "id": "tx-0105",
    "date": "2026-02-04T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Pro-labore",
    "customerId": "067.164.483-14",
    "customer": "JULIO CESAR RODR…",
    "typeId": 3,
    "type": "Debito",
    "amount": 700.0,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0106",
    "date": "2026-02-04T12:00:00",
    "description": "Compra no débito",
    "history": "Lanches para colaboradores",
    "customerId": "-",
    "customer": "FRANCISCO JOSE D…",
    "typeId": 2,
    "type": "Debito",
    "amount": 12.0,
    "accountId": 65,
    "bank": "Cora"
  },
  {
    "id": "tx-0107",
    "date": "2026-02-05T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Pro-labore",
    "customerId": "067.164.483-14",
    "customer": "JULIO CESAR RODR…",
    "typeId": 3,
    "type": "Debito",
    "amount": 200.0,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0108",
    "date": "2026-02-09T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Pro-labore",
    "customerId": "067.164.483-14",
    "customer": "JULIO CESAR RODR…",
    "typeId": 3,
    "type": "Debito",
    "amount": 150.0,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0109",
    "date": "2026-02-10T12:00:00",
    "description": "Compra no débito",
    "history": "Lanches para colaboradores",
    "customerId": "-",
    "customer": "FRANCISCO JOSE D…",
    "typeId": 2,
    "type": "Debito",
    "amount": 12.0,
    "accountId": 65,
    "bank": "Cora"
  },
  {
    "id": "tx-0110",
    "date": "2026-02-18T12:00:00",
    "description": "Compra no débito",
    "history": "Lanches para colaboradores",
    "customerId": "-",
    "customer": "JOAOPAULOMESQ…",
    "typeId": 2,
    "type": "Debito",
    "amount": 10.0,
    "accountId": 65,
    "bank": "Cora"
  },
  {
    "id": "tx-0111",
    "date": "2026-02-18T12:00:00",
    "description": "Compra no débito",
    "history": "Lanches para colaboradores",
    "customerId": "-",
    "customer": "FRANCISCO JOSE D…",
    "typeId": 2,
    "type": "Debito",
    "amount": 12.0,
    "accountId": 65,
    "bank": "Cora"
  },
  {
    "id": "tx-0112",
    "date": "2026-02-25T12:00:00",
    "description": "Transf Pix recebida",
    "history": "Transferencia entre contas",
    "customerId": "41.655.312/0001-10",
    "customer": "41.655.312 JULIO C…",
    "typeId": 1,
    "type": "Credito",
    "amount": 2497.0,
    "accountId": 106,
    "bank": "Cora"
  },
  {
    "id": "tx-0113",
    "date": "2026-02-25T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Transferencia entre contas",
    "customerId": "41.655.312/0001-10",
    "customer": "41.655.312 JULIO C…",
    "typeId": 2,
    "type": "Debito",
    "amount": 970.0,
    "accountId": 104,
    "bank": "Cora"
  },
  {
    "id": "tx-0114",
    "date": "2026-02-25T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Transferencia entre contas",
    "customerId": "41.655.312/0001-10",
    "customer": "41.655.312 JULIO C…",
    "typeId": 2,
    "type": "Debito",
    "amount": 300.0,
    "accountId": 104,
    "bank": "Cora"
  },
  {
    "id": "tx-0115",
    "date": "2026-02-25T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "Parcela Moto",
    "customerId": "10.573.521/0001-91",
    "customer": "MERCADO PAGO IN…",
    "typeId": 3,
    "type": "Debito",
    "amount": 200.0,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0116",
    "date": "2026-02-25T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "Tripé",
    "customerId": "10.573.521/0001-91",
    "customer": "MERCADO PAGO IN…",
    "typeId": 2,
    "type": "Debito",
    "amount": 49.44,
    "accountId": 73,
    "bank": "Cora"
  },
  {
    "id": "tx-0117",
    "date": "2026-02-25T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "Treinamento",
    "customerId": "10.573.521/0001-91",
    "customer": "MERCADO PAGO IN…",
    "typeId": 2,
    "type": "Debito",
    "amount": 149.7,
    "accountId": 98,
    "bank": "Cora"
  },
  {
    "id": "tx-0118",
    "date": "2026-02-25T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "Sistema",
    "customerId": "10.573.521/0001-91",
    "customer": "MERCADO PAGO IN…",
    "typeId": 3,
    "type": "Debito",
    "amount": 189.0,
    "accountId": 25,
    "bank": "Cora"
  },
  {
    "id": "tx-0119",
    "date": "2026-02-25T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "Juros e Multas",
    "customerId": "10.573.521/0001-91",
    "customer": "MERCADO PAGO IN…",
    "typeId": 2,
    "type": "Debito",
    "amount": 14.94,
    "accountId": 63,
    "bank": "Cora"
  },
  {
    "id": "tx-0120",
    "date": "2026-02-26T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "Internet",
    "customerId": "04.601.397/0001-28",
    "customer": "BRISANET SERVICO…",
    "typeId": 2,
    "type": "Debito",
    "amount": 91.95,
    "accountId": 57,
    "bank": "Cora"
  },
  {
    "id": "tx-0121",
    "date": "2026-02-26T12:00:00",
    "description": "Transf Pix recebida",
    "history": "Transferencia entre contas",
    "customerId": "41.655.312/0001-10",
    "customer": "41 655 312 JULIO C…",
    "typeId": 1,
    "type": "Credito",
    "amount": 66.05,
    "accountId": 106,
    "bank": "Cora"
  },
  {
    "id": "tx-0122",
    "date": "2026-02-26T12:00:00",
    "description": "Transf Pix recebida",
    "history": "Projetos Avulsos",
    "customerId": "093.307.123-04",
    "customer": "FRANCISCO J M LO…",
    "typeId": 1,
    "type": "Credito",
    "amount": 3000.0,
    "accountId": 2,
    "bank": "Cora"
  },
  {
    "id": "tx-0123",
    "date": "2026-02-26T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Folha de Pagamento Custo",
    "customerId": "603.804.323-31",
    "customer": "Mayane Lima dos Sa…",
    "typeId": 3,
    "type": "Debito",
    "amount": 1180.0,
    "accountId": 18,
    "bank": "Cora"
  },
  {
    "id": "tx-0124",
    "date": "2026-02-27T12:00:00",
    "description": "Compra no débito",
    "history": "Lanches para colaboradores",
    "customerId": "-",
    "customer": "JIM.COM* MERCAD…",
    "typeId": 2,
    "type": "Debito",
    "amount": 6.0,
    "accountId": 65,
    "bank": "Cora"
  },
  {
    "id": "tx-0125",
    "date": "2026-02-27T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Pro-labore",
    "customerId": "067.164.483-14",
    "customer": "JULIO CESAR RODR…",
    "typeId": 3,
    "type": "Debito",
    "amount": 100.0,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0126",
    "date": "2026-02-28T12:00:00",
    "description": "Transf Pix recebida",
    "history": "Projetos Avulsos",
    "customerId": "41.904.774/0001-24",
    "customer": "HUST COSMETICOS…",
    "typeId": 1,
    "type": "Credito",
    "amount": 975.0,
    "accountId": 2,
    "bank": "Cora"
  },
  {
    "id": "tx-0127",
    "date": "2026-02-28T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Pro-labore",
    "customerId": "067.164.483-14",
    "customer": "JULIO CESAR RODR…",
    "typeId": 3,
    "type": "Debito",
    "amount": 2020.0,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0128",
    "date": "2026-03-01T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Pro-labore",
    "customerId": "067.164.483-14",
    "customer": "JULIO CESAR RODR…",
    "typeId": 3,
    "type": "Debito",
    "amount": 100.0,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0129",
    "date": "2026-03-02T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Pro-labore",
    "customerId": "067.164.483-14",
    "customer": "JULIO CESAR RODR…",
    "typeId": 3,
    "type": "Debito",
    "amount": 100.0,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0130",
    "date": "2026-03-03T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Salários",
    "customerId": "082.939.493-17",
    "customer": "JOAO VITOR FONT…",
    "typeId": 3,
    "type": "Debito",
    "amount": 1000.0,
    "accountId": 18,
    "bank": "Cora"
  },
  {
    "id": "tx-0131",
    "date": "2026-03-03T12:00:00",
    "description": "Compra no débito",
    "history": "Lanches para colaboradores",
    "customerId": "-",
    "customer": "FRANCISCO JOSE D…",
    "typeId": 2,
    "type": "Debito",
    "amount": 6.5,
    "accountId": 65,
    "bank": "Cora"
  },
  {
    "id": "tx-0132",
    "date": "2026-03-04T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "Lanches para colaboradores",
    "customerId": "600.809.753-05",
    "customer": "Alessandro Magalhae…",
    "typeId": 2,
    "type": "Debito",
    "amount": 18.0,
    "accountId": 65,
    "bank": "Cora"
  },
  {
    "id": "tx-0133",
    "date": "2026-03-06T12:00:00",
    "description": "Compra no débito",
    "history": "",
    "customerId": "-",
    "customer": "LOJA IMPERIO.",
    "typeId": 1,
    "type": "Debito",
    "amount": 80.0,
    "accountId": 0,
    "bank": "Cora"
  },
  {
    "id": "tx-0134",
    "date": "2026-03-06T12:00:00",
    "description": "Compra no débito",
    "history": "",
    "customerId": "-",
    "customer": "LIVRARIA VASCONCE",
    "typeId": 1,
    "type": "Debito",
    "amount": 11.33,
    "accountId": 0,
    "bank": "Cora"
  },
  {
    "id": "tx-0135",
    "date": "2026-03-06T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Pro-labore",
    "customerId": "067.164.483-14",
    "customer": "JULIO CESAR RODR…",
    "typeId": 3,
    "type": "Debito",
    "amount": 50.0,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0136",
    "date": "2026-03-08T12:00:00",
    "description": "Compra no débito",
    "history": "Combustível",
    "customerId": "-",
    "customer": "CATATAU COMERCI…",
    "typeId": 2,
    "type": "Debito",
    "amount": 50.0,
    "accountId": 33,
    "bank": "Cora"
  },
  {
    "id": "tx-0137",
    "date": "2026-03-08T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Pro-labore",
    "customerId": "067.164.483-14",
    "customer": "JULIO CESAR RODR…",
    "typeId": 3,
    "type": "Debito",
    "amount": 40.0,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0138",
    "date": "2026-03-09T12:00:00",
    "description": "Transf Pix recebida",
    "history": "Projetos Avulsos",
    "customerId": "57.600.227/0001-95",
    "customer": "F. P. FROTA DE LIMA",
    "typeId": 1,
    "type": "Credito",
    "amount": 3500.0,
    "accountId": 2,
    "bank": "Cora"
  },
  {
    "id": "tx-0139",
    "date": "2026-03-10T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Folha de Pagamento Custo",
    "customerId": "603.804.323-31",
    "customer": "Mayane Lima dos Sa…",
    "typeId": 3,
    "type": "Debito",
    "amount": 1300.0,
    "accountId": 18,
    "bank": "Cora"
  },
  {
    "id": "tx-0140",
    "date": "2026-03-10T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Pro-labore",
    "customerId": "067.164.483-14",
    "customer": "JULIO CESAR RODR…",
    "typeId": 3,
    "type": "Debito",
    "amount": 1000.0,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0141",
    "date": "2026-03-10T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Folha de Pagamento Custo",
    "customerId": "082.939.493-17",
    "customer": "JOAO VITOR FONT…",
    "typeId": 3,
    "type": "Debito",
    "amount": 695.0,
    "accountId": 18,
    "bank": "Cora"
  },
  {
    "id": "tx-0142",
    "date": "2026-03-11T12:00:00",
    "description": "Compra no débito",
    "history": "Combustível",
    "customerId": "-",
    "customer": "CATATAU COMERCI…",
    "typeId": 2,
    "type": "Debito",
    "amount": 100.0,
    "accountId": 33,
    "bank": "Cora"
  },
  {
    "id": "tx-0143",
    "date": "2026-03-12T12:00:00",
    "description": "Compra no débito",
    "history": "Manutenção de Veículos",
    "customerId": "-",
    "customer": "MP*BIBILAUTOPECA",
    "typeId": 2,
    "type": "Debito",
    "amount": 200.0,
    "accountId": 70,
    "bank": "Cora"
  },
  {
    "id": "tx-0144",
    "date": "2026-03-13T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "",
    "customerId": "38.372.267/0001-82",
    "customer": "SHPP BRASIL INSTIT…",
    "typeId": 3,
    "type": "Debito",
    "amount": 22.6,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0145",
    "date": "2026-03-13T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "Lanches para colaboradores",
    "customerId": "600.809.753-05",
    "customer": "Alessandro Magalhae…",
    "typeId": 2,
    "type": "Debito",
    "amount": 10.5,
    "accountId": 65,
    "bank": "Cora"
  },
  {
    "id": "tx-0146",
    "date": "2026-03-14T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Pro-labore",
    "customerId": "067.164.483-14",
    "customer": "JULIO CESAR RODR…",
    "typeId": 3,
    "type": "Debito",
    "amount": 100.0,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0147",
    "date": "2026-03-17T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "Lanches para colaboradores",
    "customerId": "600.809.753-05",
    "customer": "Alessandro Magalhae…",
    "typeId": 2,
    "type": "Debito",
    "amount": 10.5,
    "accountId": 65,
    "bank": "Cora"
  },
  {
    "id": "tx-0148",
    "date": "2026-03-19T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "Construções/reformas",
    "customerId": "04.980.988/0001-53",
    "customer": "CELESTINO CONST…",
    "typeId": 2,
    "type": "Debito",
    "amount": 20.0,
    "accountId": 23,
    "bank": "Cora"
  },
  {
    "id": "tx-0149",
    "date": "2026-03-19T12:00:00",
    "description": "Compra no débito",
    "history": "Construções/reformas",
    "customerId": "-",
    "customer": "TECHNIC PARTICIPA",
    "typeId": 2,
    "type": "Debito",
    "amount": 12.0,
    "accountId": 23,
    "bank": "Cora"
  },
  {
    "id": "tx-0150",
    "date": "2026-03-22T12:00:00",
    "description": "Compra no débito",
    "history": "Lanches para colaboradores",
    "customerId": "-",
    "customer": "ROGERIO DO CALDO",
    "typeId": 2,
    "type": "Debito",
    "amount": 30.0,
    "accountId": 65,
    "bank": "Cora"
  },
  {
    "id": "tx-0151",
    "date": "2026-03-23T12:00:00",
    "description": "Transf Pix recebida",
    "history": "Transferencia entre contas",
    "customerId": "41.655.312/0001-10",
    "customer": "41.655.312 JULIO C…",
    "typeId": 1,
    "type": "Credito",
    "amount": 2497.06,
    "accountId": 106,
    "bank": "Cora"
  },
  {
    "id": "tx-0152",
    "date": "2026-03-23T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "Internet",
    "customerId": "04.601.397/0001-28",
    "customer": "BRISANET SERVICO…",
    "typeId": 2,
    "type": "Debito",
    "amount": 92.32,
    "accountId": 57,
    "bank": "Cora"
  },
  {
    "id": "tx-0153",
    "date": "2026-03-23T12:00:00",
    "description": "Pgto QR Code Pix",
    "history": "MEI mês atual",
    "customerId": "00.394.460/0058-87",
    "customer": "MINISTERIO DA FAZ…",
    "typeId": 2,
    "type": "Debito",
    "amount": 32.47,
    "accountId": 89,
    "bank": "Cora"
  },
  {
    "id": "tx-0154",
    "date": "2026-03-23T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Pro-labore",
    "customerId": "067.164.483-14",
    "customer": "JULIO CESAR RODR…",
    "typeId": 3,
    "type": "Debito",
    "amount": 565.94,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0155",
    "date": "2026-03-23T12:00:00",
    "description": "Compra no débito",
    "history": "Lanches para colaboradores",
    "customerId": "-",
    "customer": "XIMENES ATACAREJ…",
    "typeId": 2,
    "type": "Debito",
    "amount": 17.81,
    "accountId": 65,
    "bank": "Cora"
  },
  {
    "id": "tx-0156",
    "date": "2026-03-23T12:00:00",
    "description": "Compra no débito",
    "history": "Lanches para colaboradores",
    "customerId": "-",
    "customer": "MERCADINHO BOM…",
    "typeId": 2,
    "type": "Debito",
    "amount": 16.88,
    "accountId": 65,
    "bank": "Cora"
  },
  {
    "id": "tx-0157",
    "date": "2026-03-24T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Pro-labore",
    "customerId": "067.164.483-14",
    "customer": "JULIO CESAR RODR…",
    "typeId": 3,
    "type": "Debito",
    "amount": 50.0,
    "accountId": 103,
    "bank": ""
  },
  {
    "id": "tx-0158",
    "date": "2026-03-24T12:00:00",
    "description": "Compra no débito",
    "history": "Lanches para colaboradores",
    "customerId": "-",
    "customer": "MEGA BOLO",
    "typeId": 2,
    "type": "Debito",
    "amount": 7.0,
    "accountId": 65,
    "bank": "Cora"
  },
  {
    "id": "tx-0159",
    "date": "2026-03-25T12:00:00",
    "description": "Compra no débito",
    "history": "Lanches para colaboradores",
    "customerId": "-",
    "customer": "MERCADINHO BOM…",
    "typeId": 2,
    "type": "Debito",
    "amount": 9.0,
    "accountId": 65,
    "bank": "Cora"
  },
  {
    "id": "tx-0160",
    "date": "2026-03-26T12:00:00",
    "description": "Compra no débito",
    "history": "Construções/reformas",
    "customerId": "-",
    "customer": "CONSTRUPISO TIA…",
    "typeId": 2,
    "type": "Debito",
    "amount": 73.92,
    "accountId": 23,
    "bank": "Cora"
  },
  {
    "id": "tx-0161",
    "date": "2026-03-26T12:00:00",
    "description": "Compra no débito",
    "history": "Construções/reformas",
    "customerId": "-",
    "customer": "CONSTRUPISO TIA…",
    "typeId": 2,
    "type": "Debito",
    "amount": 27.1,
    "accountId": 23,
    "bank": "Cora"
  },
  {
    "id": "tx-0162",
    "date": "2026-03-26T12:00:00",
    "description": "Compra no débito",
    "history": "Lanches para colaboradores",
    "customerId": "-",
    "customer": "SUPERMERC SERVE …",
    "typeId": 2,
    "type": "Debito",
    "amount": 17.05,
    "accountId": 65,
    "bank": "Cora"
  },
  {
    "id": "tx-0163",
    "date": "2026-03-26T12:00:00",
    "description": "Compra no débito",
    "history": "Combustível",
    "customerId": "-",
    "customer": "POSTO CATATAU",
    "typeId": 2,
    "type": "Debito",
    "amount": 100.0,
    "accountId": 33,
    "bank": "Cora"
  },
  {
    "id": "tx-0164",
    "date": "2026-03-30T12:00:00",
    "description": "Transf Pix enviada",
    "history": "Pro-labore",
    "customerId": "067.164.483-14",
    "customer": "JULIO CESAR RODR…",
    "typeId": 3,
    "type": "Debito",
    "amount": 200.0,
    "accountId": 103,
    "bank": "Cora"
  },
  {
    "id": "tx-0165",
    "date": "2026-03-31T12:00:00",
    "description": "Compra no débito",
    "history": "Combustível",
    "customerId": "-",
    "customer": "POSTO CATATAU",
    "typeId": 2,
    "type": "Debito",
    "amount": 100.0,
    "accountId": 33,
    "bank": "Cora"
  },
  {
    "id": "tx-0166",
    "date": "2025-08-25T12:00:00",
    "description": "Cobrança recebida",
    "history": "Mensalidade",
    "customerId": "05.356.440/0001-08",
    "customer": "JLanches",
    "typeId": 1,
    "type": "Credito",
    "amount": 2500.0,
    "accountId": 3,
    "bank": "Asaas"
  },
  {
    "id": "tx-0167",
    "date": "2025-08-25T12:00:00",
    "description": "Taxa de boleto",
    "history": "Tarifas de boletos",
    "customerId": "05.356.440/0001-08",
    "customer": "JLanches",
    "typeId": 2,
    "type": "Debito",
    "amount": 0.99,
    "accountId": 92,
    "bank": "Asaas"
  },
  {
    "id": "tx-0168",
    "date": "2025-08-25T12:00:00",
    "description": "Taxa de mensageria",
    "history": "Tarifas de mensageria",
    "customerId": "05.356.440/0001-08",
    "customer": "JLanches",
    "typeId": 2,
    "type": "Debito",
    "amount": 0.99,
    "accountId": 81,
    "bank": "Asaas"
  },
  {
    "id": "tx-0169",
    "date": "2025-08-25T12:00:00",
    "description": "Transação via Pix",
    "history": "Transferencia entre contas",
    "customerId": "41.655.312/0001-10",
    "customer": "JULIO CESAR RODRIGUES PIRES",
    "typeId": 2,
    "type": "Debito",
    "amount": 2500.0,
    "accountId": 104,
    "bank": "Asaas"
  },
  {
    "id": "tx-0170",
    "date": "2025-08-26T12:00:00",
    "description": "Cobrança recebida",
    "history": "Mensalidades",
    "customerId": "26.158.183/0004-04",
    "customer": "Perolab",
    "typeId": 1,
    "type": "Credito",
    "amount": 2300.0,
    "accountId": 3,
    "bank": "Asaas"
  },
  {
    "id": "tx-0171",
    "date": "2025-08-26T12:00:00",
    "description": "Taxa de boleto",
    "history": "Tarifas de boletos",
    "customerId": "26.158.183/0004-04",
    "customer": "Perolab",
    "typeId": 2,
    "type": "Debito",
    "amount": 0.99,
    "accountId": 92,
    "bank": "Asaas"
  },
  {
    "id": "tx-0172",
    "date": "2025-08-26T12:00:00",
    "description": "Taxa de mensageria",
    "history": "Tarifas de mensageria",
    "customerId": "26.158.183/0004-04",
    "customer": "Perolab",
    "typeId": 2,
    "type": "Debito",
    "amount": 0.99,
    "accountId": 81,
    "bank": "Asaas"
  },
  {
    "id": "tx-0173",
    "date": "2025-08-26T12:00:00",
    "description": "Transação via Pix",
    "history": "Transferencia entre contas",
    "customerId": "41.655.312/0001-10",
    "customer": "JULIO CESAR RODRIGUES PIRES",
    "typeId": 2,
    "type": "Debito",
    "amount": 2299.0,
    "accountId": 104,
    "bank": "Asaas"
  },
  {
    "id": "tx-0174",
    "date": "2025-09-15T12:00:00",
    "description": "Cobrança recebida",
    "history": "Mensalidade",
    "customerId": "54.687.930/0001-94",
    "customer": "FarmaFormula",
    "typeId": 1,
    "type": "Credito",
    "amount": 2000.0,
    "accountId": 3,
    "bank": "Asaas"
  },
  {
    "id": "tx-0175",
    "date": "2025-09-15T12:00:00",
    "description": "Taxa de boleto",
    "history": "Tarifas de boletos",
    "customerId": "54.687.930/0001-94",
    "customer": "FarmaFormula",
    "typeId": 2,
    "type": "Debito",
    "amount": 0.99,
    "accountId": 92,
    "bank": "Asaas"
  },
  {
    "id": "tx-0176",
    "date": "2025-09-15T12:00:00",
    "description": "Taxa de mensageria",
    "history": "Tarifas de mensageria",
    "customerId": "54.687.930/0001-94",
    "customer": "FarmaFormula",
    "typeId": 2,
    "type": "Debito",
    "amount": 0.99,
    "accountId": 81,
    "bank": "Asaas"
  },
  {
    "id": "tx-0177",
    "date": "2025-09-17T12:00:00",
    "description": "Transação via Pix",
    "history": "Transferencia entre contas",
    "customerId": "41.655.312/0001-10",
    "customer": "JULIO CESAR RODRIGUES PIRES M",
    "typeId": 2,
    "type": "Debito",
    "amount": 1998.12,
    "accountId": 104,
    "bank": "Asaas"
  },
  {
    "id": "tx-0178",
    "date": "2025-09-23T12:00:00",
    "description": "Cobrança recebida",
    "history": "Mensalidade",
    "customerId": "05.356.440/0001-08",
    "customer": "J B Ferreira de Sá",
    "typeId": 1,
    "type": "Credito",
    "amount": 2500.0,
    "accountId": 3,
    "bank": "Asaas"
  },
  {
    "id": "tx-0179",
    "date": "2025-09-23T12:00:00",
    "description": "Taxa do Pix",
    "history": "Tarifas de boletos",
    "customerId": "05.356.440/0001-08",
    "customer": "J B Ferreira de Sá",
    "typeId": 2,
    "type": "Debito",
    "amount": 0.99,
    "accountId": 92,
    "bank": "Asaas"
  },
  {
    "id": "tx-0180",
    "date": "2025-09-23T12:00:00",
    "description": "Taxa de mensageria",
    "history": "Tarifas de mensageria",
    "customerId": "05.356.440/0001-08",
    "customer": "J B Ferreira de Sá",
    "typeId": 2,
    "type": "Debito",
    "amount": 0.99,
    "accountId": 81,
    "bank": "Asaas"
  },
  {
    "id": "tx-0181",
    "date": "2025-09-24T12:00:00",
    "description": "Transação via Pix",
    "history": "Transferencia entre contas",
    "customerId": "41.655.312/0001-10",
    "customer": "JULIO CESAR RODRIGUES PIRES M",
    "typeId": 2,
    "type": "Debito",
    "amount": 2498.02,
    "accountId": 104,
    "bank": "Asaas"
  },
  {
    "id": "tx-0182",
    "date": "2025-09-24T12:00:00",
    "description": "Cobrança recebida",
    "history": "Mensalidade",
    "customerId": "",
    "customer": "Diamond",
    "typeId": 1,
    "type": "Credito",
    "amount": 2300.0,
    "accountId": 3,
    "bank": "Asaas"
  },
  {
    "id": "tx-0183",
    "date": "2025-09-24T12:00:00",
    "description": "Taxa do Pix",
    "history": "Tarifas de boletos",
    "customerId": "",
    "customer": "Diamond",
    "typeId": 2,
    "type": "Debito",
    "amount": 0.99,
    "accountId": 92,
    "bank": "Asaas"
  },
  {
    "id": "tx-0184",
    "date": "2025-09-24T12:00:00",
    "description": "Taxa de mensageria",
    "history": "Tarifas de mensageria",
    "customerId": "",
    "customer": "Diamond",
    "typeId": 2,
    "type": "Debito",
    "amount": 0.99,
    "accountId": 81,
    "bank": "Asaas"
  },
  {
    "id": "tx-0185",
    "date": "2025-09-25T12:00:00",
    "description": "Transação via Pix",
    "history": "Transferencia entre contas",
    "customerId": "",
    "customer": "41 655 312 JULIO CESAR RODRIGUES PIRES M",
    "typeId": 2,
    "type": "Debito",
    "amount": 2298.02,
    "accountId": 104,
    "bank": "Asaas"
  },
  {
    "id": "tx-0186",
    "date": "2025-09-29T12:00:00",
    "description": "Cobrança recebida",
    "history": "Mensalidade",
    "customerId": "",
    "customer": "Casa de Cecília",
    "typeId": 1,
    "type": "Credito",
    "amount": 3000.0,
    "accountId": 3,
    "bank": "Asaas"
  },
  {
    "id": "tx-0187",
    "date": "2025-09-29T12:00:00",
    "description": "Taxa do Pix",
    "history": "Tarifas de boletos",
    "customerId": "",
    "customer": "Casa de Cecília",
    "typeId": 2,
    "type": "Debito",
    "amount": 0.99,
    "accountId": 92,
    "bank": "Asaas"
  },
  {
    "id": "tx-0188",
    "date": "2025-09-29T12:00:00",
    "description": "Taxa de mensageria",
    "history": "Tarifas de mensageria",
    "customerId": "",
    "customer": "Casa de Cecília",
    "typeId": 2,
    "type": "Debito",
    "amount": 0.99,
    "accountId": 81,
    "bank": "Asaas"
  },
  {
    "id": "tx-0189",
    "date": "2025-09-29T12:00:00",
    "description": "Transação via Pix",
    "history": "Transferencia entre contas",
    "customerId": "",
    "customer": "41 655 312 JULIO CESAR RODRIGUES PIRES M",
    "typeId": 2,
    "type": "Debito",
    "amount": 2998.02,
    "accountId": 104,
    "bank": "Asaas"
  },
  {
    "id": "tx-0190",
    "date": "2025-10-06T12:00:00",
    "description": "Estorno de transação via Pix",
    "history": "Estorno pelo banco",
    "customerId": "",
    "customer": "Estorno de transação via Pix",
    "typeId": 1,
    "type": "Credito",
    "amount": 2998.02,
    "accountId": 111,
    "bank": "Asaas"
  },
  {
    "id": "tx-0191",
    "date": "2025-10-06T12:00:00",
    "description": "Transação via Pix",
    "history": "Transferencia entre contas",
    "customerId": "",
    "customer": "41 655 312 JULIO CESAR RODRIGUES PIRES M",
    "typeId": 2,
    "type": "Debito",
    "amount": 2998.02,
    "accountId": 104,
    "bank": "Asaas"
  },
  {
    "id": "tx-0192",
    "date": "2025-10-15T12:00:00",
    "description": "Cobrança recebida",
    "history": "Mensalidade",
    "customerId": "54.687.930/0001-94",
    "customer": "FarmaFormula",
    "typeId": 1,
    "type": "Credito",
    "amount": 2000.0,
    "accountId": 3,
    "bank": "Asaas"
  },
  {
    "id": "tx-0193",
    "date": "2025-10-15T12:00:00",
    "description": "Taxa de boleto",
    "history": "Tarifas de boletos",
    "customerId": "54.687.930/0001-94",
    "customer": "FarmaFormula",
    "typeId": 2,
    "type": "Debito",
    "amount": 0.99,
    "accountId": 92,
    "bank": "Asaas"
  },
  {
    "id": "tx-0194",
    "date": "2025-10-15T12:00:00",
    "description": "Taxa de mensageria",
    "history": "Tarifas de mensageria",
    "customerId": "54.687.930/0001-94",
    "customer": "FarmaFormula",
    "typeId": 2,
    "type": "Debito",
    "amount": 0.99,
    "accountId": 81,
    "bank": "Asaas"
  },
  {
    "id": "tx-0195",
    "date": "2025-10-20T12:00:00",
    "description": "Transação via Pix",
    "history": "Transferencia entre contas",
    "customerId": "",
    "customer": "41 655 312 JULIO CESAR RODRIGUES PIRES M",
    "typeId": 2,
    "type": "Debito",
    "amount": 1998.02,
    "accountId": 104,
    "bank": "Asaas"
  },
  {
    "id": "tx-0196",
    "date": "2025-10-22T12:00:00",
    "description": "Cobrança recebida",
    "history": "Mensalidade",
    "customerId": "05.356.440/0001-08",
    "customer": "J B Ferreira de Sá",
    "typeId": 1,
    "type": "Credito",
    "amount": 2500.0,
    "accountId": 3,
    "bank": "Asaas"
  },
  {
    "id": "tx-0197",
    "date": "2025-10-22T12:00:00",
    "description": "Taxa do Pix",
    "history": "Tarifas de boletos",
    "customerId": "05.356.440/0001-08",
    "customer": "J B Ferreira de Sá",
    "typeId": 2,
    "type": "Debito",
    "amount": 1.99,
    "accountId": 92,
    "bank": "Asaas"
  },
  {
    "id": "tx-0198",
    "date": "2025-10-22T12:00:00",
    "description": "Taxa de mensageria",
    "history": "Tarifas de mensageria",
    "customerId": "05.356.440/0001-08",
    "customer": "J B Ferreira de Sá",
    "typeId": 2,
    "type": "Debito",
    "amount": 0.99,
    "accountId": 81,
    "bank": "Asaas"
  },
  {
    "id": "tx-0199",
    "date": "2025-10-24T12:00:00",
    "description": "Transação via Pix",
    "history": "Transferencia entre contas",
    "customerId": "",
    "customer": "41 655 312 JULIO CESAR RODRIGUES PIRES M",
    "typeId": 2,
    "type": "Debito",
    "amount": 2497.02,
    "accountId": 104,
    "bank": "Asaas"
  },
  {
    "id": "tx-0200",
    "date": "2025-11-07T12:00:00",
    "description": "Estorno de transação via Pix",
    "history": "Estorno pelo banco",
    "customerId": "",
    "customer": "Estorno de transação via Pix",
    "typeId": 1,
    "type": "Credito",
    "amount": 2497.02,
    "accountId": 111,
    "bank": "Asaas"
  },
  {
    "id": "tx-0201",
    "date": "2025-11-07T12:00:00",
    "description": "Transação via Pix",
    "history": "Transferencia entre contas",
    "customerId": "",
    "customer": "41 655 312 JULIO CESAR RODRIGUES PIRES M",
    "typeId": 2,
    "type": "Debito",
    "amount": 2497.02,
    "accountId": 104,
    "bank": "Asaas"
  },
  {
    "id": "tx-0202",
    "date": "2025-11-14T12:00:00",
    "description": "Cobrança recebida",
    "history": "Mensalidade",
    "customerId": "",
    "customer": "FarmaFormula",
    "typeId": 1,
    "type": "Credito",
    "amount": 2000.0,
    "accountId": 3,
    "bank": "Asaas"
  },
  {
    "id": "tx-0203",
    "date": "2025-11-14T12:00:00",
    "description": "Taxa de boleto",
    "history": "Tarifas de boletos",
    "customerId": "",
    "customer": "FarmaFormula",
    "typeId": 2,
    "type": "Debito",
    "amount": 1.99,
    "accountId": 92,
    "bank": "Asaas"
  },
  {
    "id": "tx-0204",
    "date": "2025-11-14T12:00:00",
    "description": "Taxa de mensageria",
    "history": "Tarifas de mensageria",
    "customerId": "",
    "customer": "FarmaFormula",
    "typeId": 2,
    "type": "Debito",
    "amount": 0.99,
    "accountId": 81,
    "bank": "Asaas"
  },
  {
    "id": "tx-0205",
    "date": "2025-11-17T12:00:00",
    "description": "Transação via Pix",
    "history": "Transferencia entre contas",
    "customerId": "",
    "customer": "41 655 312 JULIO CESAR RODRIGUES PIRES M",
    "typeId": 2,
    "type": "Debito",
    "amount": 1997.0,
    "accountId": 104,
    "bank": "Asaas"
  },
  {
    "id": "tx-0206",
    "date": "2025-11-25T12:00:00",
    "description": "Cobrança recebida",
    "history": "Mensalidade",
    "customerId": "05.356.440/0001-08",
    "customer": "J B Ferreira de Sá",
    "typeId": 1,
    "type": "Credito",
    "amount": 2500.0,
    "accountId": 3,
    "bank": "Asaas"
  },
  {
    "id": "tx-0207",
    "date": "2025-11-25T12:00:00",
    "description": "Taxa do Pix",
    "history": "Tarifas de boletos",
    "customerId": "05.356.440/0001-08",
    "customer": "J B Ferreira de Sá",
    "typeId": 2,
    "type": "Debito",
    "amount": 1.99,
    "accountId": 92,
    "bank": "Asaas"
  },
  {
    "id": "tx-0208",
    "date": "2025-11-25T12:00:00",
    "description": "Taxa de mensageria",
    "history": "Tarifas de mensageria",
    "customerId": "05.356.440/0001-08",
    "customer": "J B Ferreira de Sá",
    "typeId": 2,
    "type": "Debito",
    "amount": 0.99,
    "accountId": 81,
    "bank": "Asaas"
  },
  {
    "id": "tx-0209",
    "date": "2025-11-25T12:00:00",
    "description": "Transação via Pix",
    "history": "Transferencia entre contas",
    "customerId": "",
    "customer": "41 655 312 JULIO CESAR RODRIGUES PIRES M",
    "typeId": 2,
    "type": "Debito",
    "amount": 2497.0,
    "accountId": 104,
    "bank": "Asaas"
  },
  {
    "id": "tx-0210",
    "date": "2025-12-15T12:00:00",
    "description": "Cobrança recebida",
    "history": "Mensalidade",
    "customerId": "",
    "customer": "FarmaFormula",
    "typeId": 1,
    "type": "Credito",
    "amount": 2000.0,
    "accountId": 3,
    "bank": "Asaas"
  },
  {
    "id": "tx-0211",
    "date": "2025-12-15T12:00:00",
    "description": "Taxa do Pix",
    "history": "Tarifas de boletos",
    "customerId": "",
    "customer": "FarmaFormula",
    "typeId": 2,
    "type": "Debito",
    "amount": 1.99,
    "accountId": 92,
    "bank": "Asaas"
  },
  {
    "id": "tx-0212",
    "date": "2025-12-15T12:00:00",
    "description": "Taxa de mensageria",
    "history": "Tarifas de mensageria",
    "customerId": "",
    "customer": "FarmaFormula",
    "typeId": 2,
    "type": "Debito",
    "amount": 0.99,
    "accountId": 81,
    "bank": "Asaas"
  },
  {
    "id": "tx-0213",
    "date": "2025-12-16T12:00:00",
    "description": "Transação via Pix",
    "history": "Transferencia entre contas",
    "customerId": "",
    "customer": "41655312 JULIO CESAR RODRIGUES PIRES",
    "typeId": 2,
    "type": "Debito",
    "amount": 1997.06,
    "accountId": 104,
    "bank": "Asaas"
  },
  {
    "id": "tx-0214",
    "date": "2025-12-24T12:00:00",
    "description": "Cobrança recebida",
    "history": "Mensalidade",
    "customerId": "05.356.440/0001-08",
    "customer": "J B Ferreira de Sá",
    "typeId": 1,
    "type": "Credito",
    "amount": 2500.0,
    "accountId": 3,
    "bank": "Asaas"
  },
  {
    "id": "tx-0215",
    "date": "2025-12-24T12:00:00",
    "description": "Taxa do Pix",
    "history": "Tarifas de boletos",
    "customerId": "05.356.440/0001-08",
    "customer": "J B Ferreira de Sá",
    "typeId": 2,
    "type": "Debito",
    "amount": 1.99,
    "accountId": 92,
    "bank": "Asaas"
  },
  {
    "id": "tx-0216",
    "date": "2025-12-24T12:00:00",
    "description": "Taxa de mensageria",
    "history": "Tarifas de mensageria",
    "customerId": "05.356.440/0001-08",
    "customer": "J B Ferreira de Sá",
    "typeId": 2,
    "type": "Debito",
    "amount": 0.99,
    "accountId": 81,
    "bank": "Asaas"
  },
  {
    "id": "tx-0217",
    "date": "2026-01-16T12:00:00",
    "description": "Cobrança recebida",
    "history": "Mensalidade",
    "customerId": "",
    "customer": "FarmaFormula",
    "typeId": 1,
    "type": "Credito",
    "amount": 2000.0,
    "accountId": 3,
    "bank": "Asaas"
  },
  {
    "id": "tx-0218",
    "date": "2026-01-16T12:00:00",
    "description": "Taxa do Pix",
    "history": "Tarifas de boletos",
    "customerId": "",
    "customer": "FarmaFormula",
    "typeId": 2,
    "type": "Debito",
    "amount": 1.99,
    "accountId": 92,
    "bank": "Asaas"
  },
  {
    "id": "tx-0219",
    "date": "2026-01-16T12:00:00",
    "description": "Taxa de mensageria",
    "history": "Tarifas de mensageria",
    "customerId": "",
    "customer": "FarmaFormula",
    "typeId": 2,
    "type": "Debito",
    "amount": 0.99,
    "accountId": 81,
    "bank": "Asaas"
  },
  {
    "id": "tx-0220",
    "date": "2026-01-19T12:00:00",
    "description": "Transação via Pix",
    "history": "Transferencia entre contas",
    "customerId": "",
    "customer": "41 655 312 JULIO CESAR RODRIGUES PIRES M",
    "typeId": 2,
    "type": "Debito",
    "amount": 1997.02,
    "accountId": 104,
    "bank": "Asaas"
  },
  {
    "id": "tx-0221",
    "date": "2026-01-19T12:00:00",
    "description": "Estorno de transação via Pix",
    "history": "Estorno pelo banco",
    "customerId": "",
    "customer": "Estorno de transação via Pix",
    "typeId": 1,
    "type": "Credito",
    "amount": 1997.02,
    "accountId": 111,
    "bank": "Asaas"
  },
  {
    "id": "tx-0222",
    "date": "2026-01-19T12:00:00",
    "description": "Transação via Pix",
    "history": "Transferencia entre contas",
    "customerId": "",
    "customer": "41 655 312 JULIO CESAR RODRIGUES PIRES M",
    "typeId": 2,
    "type": "Debito",
    "amount": 1997.02,
    "accountId": 104,
    "bank": "Asaas"
  },
  {
    "id": "tx-0223",
    "date": "2026-01-22T12:00:00",
    "description": "Cobrança recebida",
    "history": "Mensalidade",
    "customerId": "05.356.440/0001-08",
    "customer": "J B Ferreira de Sá",
    "typeId": 1,
    "type": "Credito",
    "amount": 2500.0,
    "accountId": 3,
    "bank": "Asaas"
  },
  {
    "id": "tx-0224",
    "date": "2026-01-22T12:00:00",
    "description": "Taxa do Pix",
    "history": "Tarifas de boletos",
    "customerId": "05.356.440/0001-08",
    "customer": "J B Ferreira de Sá",
    "typeId": 2,
    "type": "Debito",
    "amount": 1.99,
    "accountId": 92,
    "bank": "Asaas"
  },
  {
    "id": "tx-0225",
    "date": "2026-01-22T12:00:00",
    "description": "Taxa de mensageria",
    "history": "Tarifas de mensageria",
    "customerId": "05.356.440/0001-08",
    "customer": "J B Ferreira de Sá",
    "typeId": 2,
    "type": "Debito",
    "amount": 0.99,
    "accountId": 81,
    "bank": "Asaas"
  },
  {
    "id": "tx-0226",
    "date": "2026-01-23T12:00:00",
    "description": "Transação via Pix",
    "history": "Transferencia entre contas",
    "customerId": "",
    "customer": "41 655 312 JULIO CESAR RODRIGUES PIRES M",
    "typeId": 2,
    "type": "Debito",
    "amount": 2497.0,
    "accountId": 104,
    "bank": "Asaas"
  },
  {
    "id": "tx-0227",
    "date": "2026-02-24T12:00:00",
    "description": "Cobrança recebida",
    "history": "Mensalidade",
    "customerId": "05.356.440/0001-08",
    "customer": "J B Ferreira de Sá",
    "typeId": 1,
    "type": "Credito",
    "amount": 2500.0,
    "accountId": 3,
    "bank": "Asaas"
  },
  {
    "id": "tx-0228",
    "date": "2026-02-24T12:00:00",
    "description": "Taxa do Pix",
    "history": "Tarifas de boletos",
    "customerId": "05.356.440/0001-08",
    "customer": "J B Ferreira de Sá",
    "typeId": 2,
    "type": "Debito",
    "amount": 1.99,
    "accountId": 92,
    "bank": "Asaas"
  },
  {
    "id": "tx-0229",
    "date": "2026-02-24T12:00:00",
    "description": "Taxa de mensageria",
    "history": "Tarifas de mensageria",
    "customerId": "05.356.440/0001-08",
    "customer": "J B Ferreira de Sá",
    "typeId": 2,
    "type": "Debito",
    "amount": 0.99,
    "accountId": 81,
    "bank": "Asaas"
  },
  {
    "id": "tx-0230",
    "date": "2026-02-25T12:00:00",
    "description": "Transação via Pix",
    "history": "Transferencia entre contas",
    "customerId": "",
    "customer": "41 655 312 JULIO CESAR RODRIGUES PIRES M",
    "typeId": 2,
    "type": "Debito",
    "amount": 2497.0,
    "accountId": 104,
    "bank": "Asaas"
  },
  {
    "id": "tx-0231",
    "date": "2026-03-23T12:00:00",
    "description": "Cobrança recebida",
    "history": "Mensalidade",
    "customerId": "05.356.440/0001-08",
    "customer": "J B Ferreira de Sá",
    "typeId": 1,
    "type": "Credito",
    "amount": 2500.0,
    "accountId": 3,
    "bank": "Asaas"
  },
  {
    "id": "tx-0232",
    "date": "2026-03-23T12:00:00",
    "description": "Taxa do Pix",
    "history": "Tarifas de boletos",
    "customerId": "05.356.440/0001-08",
    "customer": "J B Ferreira de Sá",
    "typeId": 2,
    "type": "Debito",
    "amount": 1.99,
    "accountId": 92,
    "bank": "Asaas"
  },
  {
    "id": "tx-0233",
    "date": "2026-03-23T12:00:00",
    "description": "Taxa de mensageria",
    "history": "Tarifas de mensageria",
    "customerId": "05.356.440/0001-08",
    "customer": "J B Ferreira de Sá",
    "typeId": 2,
    "type": "Debito",
    "amount": 0.99,
    "accountId": 81,
    "bank": "Asaas"
  },
  {
    "id": "tx-0234",
    "date": "2026-03-23T12:00:00",
    "description": "Transação via Pix",
    "history": "Transferencia entre contas",
    "customerId": "",
    "customer": "41 655 312 JULIO CESAR RODRIGUES PIRES M",
    "typeId": 2,
    "type": "Debito",
    "amount": 2497.06,
    "accountId": 104,
    "bank": "Asaas"
  },
  {
    "id": "tx-0235",
    "date": "2025-09-10T12:00:00",
    "description": "PIX RECEBIDO",
    "history": "Transferencias entre contas",
    "customerId": "41.655.312/0001-10",
    "customer": "41.655.312 JULIO CESAR RODRIGUES PIRES",
    "typeId": 1,
    "type": "Credito",
    "amount": 30.0,
    "accountId": 106,
    "bank": "Santander"
  },
  {
    "id": "tx-0236",
    "date": "2025-09-10T12:00:00",
    "description": "PIX RECEBIDO",
    "history": "Transferencias entre contas",
    "customerId": "41.655.312/0001-10",
    "customer": "41.655.312 JULIO CESAR RODRIGUES PIRES",
    "typeId": 1,
    "type": "Credito",
    "amount": 30.0,
    "accountId": 106,
    "bank": "Santander"
  },
  {
    "id": "tx-0237",
    "date": "2025-09-10T12:00:00",
    "description": "PIX RECEBIDO",
    "history": "Transferencias entre contas",
    "customerId": "41.655.312/0001-10",
    "customer": "41.655.312 JULIO CESAR RODRIGUES PIRES",
    "typeId": 1,
    "type": "Credito",
    "amount": 30.0,
    "accountId": 106,
    "bank": "Santander"
  },
  {
    "id": "tx-0238",
    "date": "2025-09-11T12:00:00",
    "description": "COMPRA CARTAO DEB",
    "history": "Combustivel",
    "customerId": "",
    "customer": "POSTO CATATAU VIII",
    "typeId": 2,
    "type": "Debito",
    "amount": 100.0,
    "accountId": 33,
    "bank": "Santander"
  },
  {
    "id": "tx-0239",
    "date": "2025-09-11T12:00:00",
    "description": "PIX RECEBIDO",
    "history": "Transferencias entre contas",
    "customerId": "41.655.312/0001-10",
    "customer": "41.655.312 JULIO CESAR RODRIGUES PIRES",
    "typeId": 1,
    "type": "Credito",
    "amount": 7129.65,
    "accountId": 106,
    "bank": "Santander"
  },
  {
    "id": "tx-0240",
    "date": "2025-09-12T12:00:00",
    "description": "COMPRA CARTAO DEB",
    "history": "Lanches para colaboradores",
    "customerId": "",
    "customer": "MEGA BOLO",
    "typeId": 2,
    "type": "Debito",
    "amount": 5.0,
    "accountId": 65,
    "bank": "Lanches para colaboradores"
  },
  {
    "id": "tx-0241",
    "date": "2025-09-15T12:00:00",
    "description": "COMPRA CARTAO DEB",
    "history": "Lanches para colaboradores",
    "customerId": "",
    "customer": "FRANCISCO JOSE DE S",
    "typeId": 2,
    "type": "Debito",
    "amount": 6.5,
    "accountId": 65,
    "bank": "Santander"
  },
  {
    "id": "tx-0242",
    "date": "2025-09-15T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Pilhas",
    "customerId": "",
    "customer": "AMAZON COM BR",
    "typeId": 2,
    "type": "Debito",
    "amount": 46.94,
    "accountId": 44,
    "bank": "Santander"
  },
  {
    "id": "tx-0243",
    "date": "2025-09-15T12:00:00",
    "description": "COMPRA CARTAO DEB",
    "history": "Combustivel",
    "customerId": "",
    "customer": "POSTO CATATAU",
    "typeId": 2,
    "type": "Debito",
    "amount": 150.0,
    "accountId": 33,
    "bank": "Santander"
  },
  {
    "id": "tx-0244",
    "date": "2025-09-15T12:00:00",
    "description": "PIX RECEBIDO",
    "history": "Esporadico",
    "customerId": "6780700320",
    "customer": "JOSE INACIO SOUSA ALBUQUERQUE",
    "typeId": 1,
    "type": "Credito",
    "amount": 650.0,
    "accountId": 2,
    "bank": "Santander"
  },
  {
    "id": "tx-0245",
    "date": "2025-09-17T12:00:00",
    "description": "COMPRA CARTAO DEB",
    "history": "Copos descartaveis",
    "customerId": "",
    "customer": "MP .TEIXEIRASACOL",
    "typeId": 2,
    "type": "Debito",
    "amount": 5.2,
    "accountId": 102,
    "bank": "Santander"
  },
  {
    "id": "tx-0246",
    "date": "2025-09-17T12:00:00",
    "description": "COMPRA CARTAO DEB",
    "history": "Lanches para colaboradores",
    "customerId": "",
    "customer": "LIPI SUPERMERCADO",
    "typeId": 2,
    "type": "Debito",
    "amount": 4.39,
    "accountId": 65,
    "bank": "Santander"
  },
  {
    "id": "tx-0247",
    "date": "2025-09-17T12:00:00",
    "description": "PIX RECEBIDO",
    "history": "Transferencias entre contas",
    "customerId": "41.655.312/0001-10",
    "customer": "41.655.312 JULIO CESAR RODRIGUES PIRES",
    "typeId": 1,
    "type": "Credito",
    "amount": 1998.12,
    "accountId": 106,
    "bank": "Santander"
  },
  {
    "id": "tx-0248",
    "date": "2025-09-18T12:00:00",
    "description": "COMPRA CARTAO DEB",
    "history": "Lanches para colaboradores",
    "customerId": "",
    "customer": "R G MOREIRA SOUZA C",
    "typeId": 2,
    "type": "Debito",
    "amount": 7.49,
    "accountId": 65,
    "bank": "Santander"
  },
  {
    "id": "tx-0249",
    "date": "2025-09-19T12:00:00",
    "description": "PIX RECEBIDO",
    "history": "Esporadico",
    "customerId": "99256177587",
    "customer": "DANILA C M CERQUEIRA",
    "typeId": 1,
    "type": "Credito",
    "amount": 1000.0,
    "accountId": 2,
    "bank": "Santander"
  },
  {
    "id": "tx-0250",
    "date": "2025-09-22T12:00:00",
    "description": "PAGAMENTO DE BOLETO",
    "history": "BPO Ficeiro",
    "customerId": "13.438.336/0001-82",
    "customer": "ISRAEL FROTA ASSESSORIA",
    "typeId": 2,
    "type": "Debito",
    "amount": 300.0,
    "accountId": 24,
    "bank": "Santander"
  },
  {
    "id": "tx-0251",
    "date": "2025-09-22T12:00:00",
    "description": "PAGAMENTO DARF EM CANAIS INTERNET",
    "history": "MEI atual e mês atrasado",
    "customerId": "00.394.460/0058-87",
    "customer": "SIMPLES NACIONAL",
    "typeId": 2,
    "type": "Debito",
    "amount": 182.57,
    "accountId": 89,
    "bank": "Santander"
  },
  {
    "id": "tx-0252",
    "date": "2025-09-22T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Gaveteiros",
    "customerId": "",
    "customer": "MERCADO PAGO INSTITUICAO",
    "typeId": 2,
    "type": "Debito",
    "amount": 66.93,
    "accountId": 77,
    "bank": "Santander"
  },
  {
    "id": "tx-0253",
    "date": "2025-09-22T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Ar Condicionado",
    "customerId": "",
    "customer": "MERCADO PAGO INSTITUICAO",
    "typeId": 2,
    "type": "Debito",
    "amount": 291.83,
    "accountId": 73,
    "bank": "Santander"
  },
  {
    "id": "tx-0254",
    "date": "2025-09-22T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Tripé",
    "customerId": "",
    "customer": "MERCADO PAGO INSTITUICAO",
    "typeId": 2,
    "type": "Debito",
    "amount": 49.44,
    "accountId": 73,
    "bank": "Santander"
  },
  {
    "id": "tx-0255",
    "date": "2025-09-22T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Trieinamento",
    "customerId": "",
    "customer": "MERCADO PAGO INSTITUICAO",
    "typeId": 2,
    "type": "Debito",
    "amount": 149.7,
    "accountId": 98,
    "bank": "Santander"
  },
  {
    "id": "tx-0256",
    "date": "2025-09-22T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Programa de Edição",
    "customerId": "",
    "customer": "MERCADO PAGO INSTITUICAO",
    "typeId": 3,
    "type": "Debito",
    "amount": 95.0,
    "accountId": 25,
    "bank": "Santander"
  },
  {
    "id": "tx-0257",
    "date": "2025-09-22T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Tarifas de cartão",
    "customerId": "",
    "customer": "MERCADO PAGO INSTITUICAO",
    "typeId": 2,
    "type": "Debito",
    "amount": 20.94,
    "accountId": 94,
    "bank": "Santander"
  },
  {
    "id": "tx-0258",
    "date": "2025-09-22T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Pró-labore",
    "customerId": "07.256.867/0016-38",
    "customer": "ARES MOTOS",
    "typeId": 3,
    "type": "Debito",
    "amount": 4400.0,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0259",
    "date": "2025-09-24T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Internet",
    "customerId": "04.601.397/0001-28",
    "customer": "BRISANET",
    "typeId": 2,
    "type": "Debito",
    "amount": 86.92,
    "accountId": 57,
    "bank": "Santander"
  },
  {
    "id": "tx-0260",
    "date": "2025-09-24T12:00:00",
    "description": "PIX RECEBIDO",
    "history": "Transferencias entre contas",
    "customerId": "41.655.312/0001-10",
    "customer": "41 655 312 JULIO CESAR RO",
    "typeId": 1,
    "type": "Credito",
    "amount": 2498.02,
    "accountId": 106,
    "bank": "Santander"
  },
  {
    "id": "tx-0261",
    "date": "2025-09-26T12:00:00",
    "description": "PIX RECEBIDO",
    "history": "Esporadico",
    "customerId": "",
    "customer": "DANILA C M CERQUEIRA",
    "typeId": 1,
    "type": "Credito",
    "amount": 1000.0,
    "accountId": 2,
    "bank": "Santander"
  },
  {
    "id": "tx-0262",
    "date": "2025-09-26T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Pró-labore",
    "customerId": "",
    "customer": "XIMENES ATACAREJO",
    "typeId": 3,
    "type": "Debito",
    "amount": 25.31,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0263",
    "date": "2025-09-29T12:00:00",
    "description": "PIX RECEBIDO",
    "history": "Esporadico",
    "customerId": "",
    "customer": "DANILA C M CERQUEIRA",
    "typeId": 1,
    "type": "Credito",
    "amount": 300.0,
    "accountId": 2,
    "bank": "Santander"
  },
  {
    "id": "tx-0264",
    "date": "2025-09-29T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Pró-labore",
    "customerId": "",
    "customer": "JULIO CESAR RODRIGUES PIR",
    "typeId": 3,
    "type": "Debito",
    "amount": 1637.77,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0265",
    "date": "2025-10-01T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Salários",
    "customerId": "603.804.323-31",
    "customer": "Mayane Lima dos Santos",
    "typeId": 3,
    "type": "Debito",
    "amount": 1500.0,
    "accountId": 18,
    "bank": "Santander"
  },
  {
    "id": "tx-0266",
    "date": "2025-10-01T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Reembolso Combustivel",
    "customerId": "603.804.323-31",
    "customer": "Mayane Lima dos Santos",
    "typeId": 2,
    "type": "Debito",
    "amount": 100.0,
    "accountId": 33,
    "bank": "Santander"
  },
  {
    "id": "tx-0267",
    "date": "2025-10-02T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Salários",
    "customerId": "",
    "customer": "JOAO VITOR FONTENELE DA S",
    "typeId": 3,
    "type": "Debito",
    "amount": 1500.0,
    "accountId": 18,
    "bank": "Santander"
  },
  {
    "id": "tx-0268",
    "date": "2025-10-02T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Bolsa Estágio",
    "customerId": "",
    "customer": "Saimon Nascimento Silva",
    "typeId": 3,
    "type": "Debito",
    "amount": 800.0,
    "accountId": 16,
    "bank": "Santander"
  },
  {
    "id": "tx-0269",
    "date": "2025-10-02T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Impostos MEI",
    "customerId": "",
    "customer": "RECEITA FEDERAL",
    "typeId": 2,
    "type": "Debito",
    "amount": 99.82,
    "accountId": 89,
    "bank": "Santander"
  },
  {
    "id": "tx-0270",
    "date": "2025-10-02T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Impostos MEI",
    "customerId": "",
    "customer": "RECEITA FEDERAL",
    "typeId": 2,
    "type": "Debito",
    "amount": 100.85,
    "accountId": 89,
    "bank": "Santander"
  },
  {
    "id": "tx-0271",
    "date": "2025-10-02T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Impostos MEI",
    "customerId": "",
    "customer": "RECEITA FEDERAL",
    "typeId": 2,
    "type": "Debito",
    "amount": 101.74,
    "accountId": 89,
    "bank": "Santander"
  },
  {
    "id": "tx-0272",
    "date": "2025-10-03T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Pró-labore",
    "customerId": "",
    "customer": "JULIO CESAR RODRIGUES PIR",
    "typeId": 3,
    "type": "Debito",
    "amount": 1000.0,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0273",
    "date": "2025-10-08T12:00:00",
    "description": "PIX RECEBIDO",
    "history": "Transferencias entre contas",
    "customerId": "",
    "customer": "41 655 312 JULIO CESAR RO",
    "typeId": 1,
    "type": "Credito",
    "amount": 2998.02,
    "accountId": 106,
    "bank": "Santander"
  },
  {
    "id": "tx-0274",
    "date": "2025-10-08T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Lanches e Refeições",
    "customerId": "",
    "customer": "MERCADINHO BOM PRECO",
    "typeId": 2,
    "type": "Debito",
    "amount": 8.0,
    "accountId": 65,
    "bank": "Santander"
  },
  {
    "id": "tx-0275",
    "date": "2025-10-08T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Lanches e Refeições",
    "customerId": "",
    "customer": "EMPORIO GIULIETTA",
    "typeId": 2,
    "type": "Debito",
    "amount": 27.81,
    "accountId": 65,
    "bank": "Santander"
  },
  {
    "id": "tx-0276",
    "date": "2025-10-09T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Pró-labore",
    "customerId": "",
    "customer": "JULIO CESAR RODRIGUES PIR",
    "typeId": 3,
    "type": "Debito",
    "amount": 3000.0,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0277",
    "date": "2025-10-10T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Aniversário do Joao Vitor",
    "customerId": "",
    "customer": "SUPER MAX",
    "typeId": 2,
    "type": "Debito",
    "amount": 61.75,
    "accountId": 65,
    "bank": "Santander"
  },
  {
    "id": "tx-0278",
    "date": "2025-10-17T12:00:00",
    "description": "COMPRA CARTAO DEB",
    "history": "Combustivel",
    "customerId": "",
    "customer": "POSTO CATATAU",
    "typeId": 2,
    "type": "Debito",
    "amount": 200.0,
    "accountId": 33,
    "bank": "Santander"
  },
  {
    "id": "tx-0279",
    "date": "2025-10-17T12:00:00",
    "description": "COMPRA CARTAO DEB",
    "history": "Lanches para colaboradores",
    "customerId": "",
    "customer": "PANIFICADORA PAO ME",
    "typeId": 2,
    "type": "Debito",
    "amount": 28.5,
    "accountId": 65,
    "bank": "Santander"
  },
  {
    "id": "tx-0280",
    "date": "2025-10-20T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Pinceis, folhas...",
    "customerId": "",
    "customer": "LIVRARIA VASCONCELOS LTDA",
    "typeId": 2,
    "type": "Debito",
    "amount": 57.95,
    "accountId": 75,
    "bank": "Santander"
  },
  {
    "id": "tx-0281",
    "date": "2025-10-20T12:00:00",
    "description": "PIX RECEBIDO",
    "history": "Transferencias entre contas",
    "customerId": "",
    "customer": "41 655 312 JULIO CESAR RO",
    "typeId": 1,
    "type": "Credito",
    "amount": 1998.02,
    "accountId": 106,
    "bank": "Santander"
  },
  {
    "id": "tx-0282",
    "date": "2025-10-20T12:00:00",
    "description": "PAGAMENTO CARTAO CREDITO",
    "history": "Pró-labore",
    "customerId": "",
    "customer": "",
    "typeId": 3,
    "type": "Debito",
    "amount": 851.76,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0283",
    "date": "2025-10-20T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Ar Condicionado",
    "customerId": "10.573.521/0001-91",
    "customer": "MERCADO PAGO INSTITUICAO",
    "typeId": 2,
    "type": "Debito",
    "amount": 291.83,
    "accountId": 73,
    "bank": "Santander"
  },
  {
    "id": "tx-0284",
    "date": "2025-10-20T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Tripé",
    "customerId": "",
    "customer": "MERCADO PAGO INSTITUICAO",
    "typeId": 2,
    "type": "Debito",
    "amount": 49.44,
    "accountId": 73,
    "bank": "Santander"
  },
  {
    "id": "tx-0285",
    "date": "2025-10-20T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Trieinamento",
    "customerId": "",
    "customer": "MERCADO PAGO INSTITUICAO",
    "typeId": 2,
    "type": "Debito",
    "amount": 149.7,
    "accountId": 98,
    "bank": "Santander"
  },
  {
    "id": "tx-0286",
    "date": "2025-10-20T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Programa de Edição",
    "customerId": "",
    "customer": "MERCADO PAGO INSTITUICAO",
    "typeId": 3,
    "type": "Debito",
    "amount": 95.0,
    "accountId": 25,
    "bank": "Santander"
  },
  {
    "id": "tx-0287",
    "date": "2025-10-20T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Parcela Moto",
    "customerId": "",
    "customer": "MERCADO PAGO INSTITUICAO",
    "typeId": 3,
    "type": "Debito",
    "amount": 200.0,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0288",
    "date": "2025-10-20T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Internet",
    "customerId": "",
    "customer": "BRISANET",
    "typeId": 2,
    "type": "Debito",
    "amount": 87.04,
    "accountId": 57,
    "bank": "Santander"
  },
  {
    "id": "tx-0289",
    "date": "2025-10-20T12:00:00",
    "description": "PIX ENVIADO",
    "history": "",
    "customerId": "",
    "customer": "A R C EMBALAGEM",
    "typeId": 2,
    "type": "Debito",
    "amount": 13.5,
    "accountId": 102,
    "bank": "Santander"
  },
  {
    "id": "tx-0290",
    "date": "2025-10-20T12:00:00",
    "description": "PIX RECEBIDO",
    "history": "Esporadico",
    "customerId": "",
    "customer": "Leticia de Oliveira Silva",
    "typeId": 1,
    "type": "Credito",
    "amount": 500.0,
    "accountId": 2,
    "bank": "Santander"
  },
  {
    "id": "tx-0291",
    "date": "2025-10-20T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Lanches para colaboradores",
    "customerId": "",
    "customer": "Arthur Roberto Coelho de",
    "typeId": 2,
    "type": "Debito",
    "amount": 7.0,
    "accountId": 65,
    "bank": "Santander"
  },
  {
    "id": "tx-0292",
    "date": "2025-10-21T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Lanches para colaboradores",
    "customerId": "",
    "customer": "MERCADINHO BOM PRECO",
    "typeId": 2,
    "type": "Debito",
    "amount": 8.5,
    "accountId": 65,
    "bank": "Santander"
  },
  {
    "id": "tx-0293",
    "date": "2025-10-21T12:00:00",
    "description": "PIX RECEBIDO",
    "history": "Mensalidades",
    "customerId": "",
    "customer": "SANTOS E CUNHA LTDA",
    "typeId": 1,
    "type": "Credito",
    "amount": 2300.0,
    "accountId": 3,
    "bank": "Santander"
  },
  {
    "id": "tx-0294",
    "date": "2025-10-21T12:00:00",
    "description": "PIX RECEBIDO",
    "history": "Mensalidades",
    "customerId": "",
    "customer": "SANTOS E CUNHA LTDA",
    "typeId": 1,
    "type": "Credito",
    "amount": 300.0,
    "accountId": 3,
    "bank": "Santander"
  },
  {
    "id": "tx-0295",
    "date": "2025-10-21T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Pró-labore",
    "customerId": "",
    "customer": "JULIO CESAR RODRIGUES PIR",
    "typeId": 3,
    "type": "Debito",
    "amount": 50.0,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0296",
    "date": "2025-10-23T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Pró-labore",
    "customerId": "",
    "customer": "JULIO CESAR RODRIGUES PIR",
    "typeId": 3,
    "type": "Debito",
    "amount": 500.0,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0297",
    "date": "2025-10-23T12:00:00",
    "description": "COMPRA CARTAO DEB",
    "history": "Material de Higiene e Limpeza",
    "customerId": "",
    "customer": "MATEUS SUPERMERC",
    "typeId": 2,
    "type": "Debito",
    "amount": 76.88,
    "accountId": 74,
    "bank": "Santander"
  },
  {
    "id": "tx-0298",
    "date": "2025-10-23T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Pró-labore",
    "customerId": "",
    "customer": "JULIO CESAR RODRIGUES PIR",
    "typeId": 3,
    "type": "Debito",
    "amount": 200.0,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0299",
    "date": "2025-10-27T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Pró-labore",
    "customerId": "",
    "customer": "JULIO CESAR RODRIGUES PIR",
    "typeId": 3,
    "type": "Debito",
    "amount": 200.0,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0300",
    "date": "2025-10-27T12:00:00",
    "description": "PIX RECEBIDO",
    "history": "Esporadico",
    "customerId": "",
    "customer": "VIVAZ CONSTRUTORA",
    "typeId": 1,
    "type": "Credito",
    "amount": 900.0,
    "accountId": 2,
    "bank": "Santander"
  },
  {
    "id": "tx-0301",
    "date": "2025-10-27T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Pró-labore",
    "customerId": "",
    "customer": "JULIO CESAR RODRIGUES PIR",
    "typeId": 3,
    "type": "Debito",
    "amount": 2000.0,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0302",
    "date": "2025-10-27T12:00:00",
    "description": "COMPRA CARTAO DEB",
    "history": "Lanches para colaboradores",
    "customerId": "",
    "customer": "IDEAL DELICATESSEN",
    "typeId": 2,
    "type": "Debito",
    "amount": 8.0,
    "accountId": 65,
    "bank": "Santander"
  },
  {
    "id": "tx-0303",
    "date": "2025-10-30T12:00:00",
    "description": "COMPRA CARTAO DEB",
    "history": "Lanches para colaboradores",
    "customerId": "",
    "customer": "IDEAL DELICATESSEN",
    "typeId": 2,
    "type": "Debito",
    "amount": 4.0,
    "accountId": 65,
    "bank": "Santander"
  },
  {
    "id": "tx-0304",
    "date": "2025-11-03T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Lanches para colaboradores",
    "customerId": "",
    "customer": "ANA CRISTINA PINTO DE AGU",
    "typeId": 2,
    "type": "Debito",
    "amount": 8.0,
    "accountId": 65,
    "bank": "Santander"
  },
  {
    "id": "tx-0305",
    "date": "2025-11-03T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Lanches para colaboradores",
    "customerId": "82387",
    "customer": "ANA CRISTINA PINTO DE AGU",
    "typeId": 2,
    "type": "Debito",
    "amount": 0.5,
    "accountId": 65,
    "bank": "Santander"
  },
  {
    "id": "tx-0306",
    "date": "2025-11-03T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Lanches para colaboradores",
    "customerId": "",
    "customer": "ANA CRISTINA PINTO DE AGU",
    "typeId": 2,
    "type": "Debito",
    "amount": 8.0,
    "accountId": 65,
    "bank": "Santander"
  },
  {
    "id": "tx-0307",
    "date": "2025-11-03T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Lanches para colaboradores",
    "customerId": "",
    "customer": "ANA CRISTINA PINTO DE AGU",
    "typeId": 2,
    "type": "Debito",
    "amount": 0.5,
    "accountId": 65,
    "bank": "Santander"
  },
  {
    "id": "tx-0308",
    "date": "2025-11-03T12:00:00",
    "description": "COMPRA CARTAO DEB MC",
    "history": "Lanches para colaboradores",
    "customerId": "",
    "customer": "IDEAL DELICATESSEN",
    "typeId": 2,
    "type": "Debito",
    "amount": 8.96,
    "accountId": 65,
    "bank": "Santander"
  },
  {
    "id": "tx-0309",
    "date": "2025-11-03T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Salários",
    "customerId": "",
    "customer": "JOAO VITOR FONTENELE DA S",
    "typeId": 3,
    "type": "Debito",
    "amount": 1500.0,
    "accountId": 18,
    "bank": "Santander"
  },
  {
    "id": "tx-0310",
    "date": "2025-11-03T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Salários",
    "customerId": "",
    "customer": "Mayane Lima dos Santos",
    "typeId": 3,
    "type": "Debito",
    "amount": 1500.0,
    "accountId": 18,
    "bank": "Santander"
  },
  {
    "id": "tx-0311",
    "date": "2025-11-03T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Bolsa Estágio",
    "customerId": "",
    "customer": "Saimon Nascimento Silva",
    "typeId": 3,
    "type": "Debito",
    "amount": 800.0,
    "accountId": 16,
    "bank": "Santander"
  },
  {
    "id": "tx-0312",
    "date": "2025-11-04T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Pró-labore",
    "customerId": "",
    "customer": "JULIO CESAR RODRIGUES PIR",
    "typeId": 3,
    "type": "Debito",
    "amount": 500.0,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0313",
    "date": "2025-11-04T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Impostos MEI",
    "customerId": "",
    "customer": "RECEITA FEDERAL",
    "typeId": 2,
    "type": "Debito",
    "amount": 80.9,
    "accountId": 89,
    "bank": "Santander"
  },
  {
    "id": "tx-0314",
    "date": "2025-11-05T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Pró-labore",
    "customerId": "",
    "customer": "JULIO CESAR RODRIGUES PIR",
    "typeId": 3,
    "type": "Debito",
    "amount": 115.0,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0315",
    "date": "2025-11-06T12:00:00",
    "description": "COMPRA CARTAO DEB MC 06/11",
    "history": "Lanches para colaboradores",
    "customerId": "",
    "customer": "FRANCISCO JOSE DE S",
    "typeId": 2,
    "type": "Debito",
    "amount": 8.0,
    "accountId": 65,
    "bank": "Santander"
  },
  {
    "id": "tx-0316",
    "date": "2025-11-07T12:00:00",
    "description": "PIX RECEBIDO",
    "history": "Transferencias entre contas",
    "customerId": "",
    "customer": "41 655 312 JULIO CESAR RO",
    "typeId": 1,
    "type": "Credito",
    "amount": 2497.02,
    "accountId": 106,
    "bank": "Santander"
  },
  {
    "id": "tx-0317",
    "date": "2025-11-07T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Pró-labore",
    "customerId": "",
    "customer": "JULIO CESAR RODRIGUES PIR",
    "typeId": 3,
    "type": "Debito",
    "amount": 1500.0,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0318",
    "date": "2025-11-10T12:00:00",
    "description": "COMPRA CARTAO",
    "history": "Pro-labore",
    "customerId": "",
    "customer": "EXTRA SAO JOSE",
    "typeId": 3,
    "type": "Debito",
    "amount": 49.99,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0319",
    "date": "2025-11-10T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Pró-labore",
    "customerId": "",
    "customer": "JULIO CESAR RODRIGUES PIR",
    "typeId": 3,
    "type": "Debito",
    "amount": 300.0,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0320",
    "date": "2025-11-10T12:00:00",
    "description": "PIX RECEBIDO",
    "history": "Mensalidades",
    "customerId": "",
    "customer": "CASA DE CECILIA",
    "typeId": 1,
    "type": "Credito",
    "amount": 3000.0,
    "accountId": 3,
    "bank": "Santander"
  },
  {
    "id": "tx-0321",
    "date": "2025-11-10T12:00:00",
    "description": "PIX RECEBIDO",
    "history": "Esporadico",
    "customerId": "",
    "customer": "VIVAZ CONSTRUTORA",
    "typeId": 1,
    "type": "Credito",
    "amount": 900.0,
    "accountId": 2,
    "bank": "Santander"
  },
  {
    "id": "tx-0322",
    "date": "2025-11-11T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Internet",
    "customerId": "",
    "customer": "BRISANET",
    "typeId": 2,
    "type": "Debito",
    "amount": 86.92,
    "accountId": 57,
    "bank": "Santander"
  },
  {
    "id": "tx-0323",
    "date": "2025-11-11T12:00:00",
    "description": "COMPRA CARTAO",
    "history": "Lanches para colaboradores",
    "customerId": "",
    "customer": "IDEAL DELICATESSEN",
    "typeId": 2,
    "type": "Debito",
    "amount": 7.2,
    "accountId": 65,
    "bank": "Santander"
  },
  {
    "id": "tx-0324",
    "date": "2025-11-12T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Pró-labore",
    "customerId": "",
    "customer": "JULIO CESAR RODRIGUES PIR",
    "typeId": 3,
    "type": "Debito",
    "amount": 700.0,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0325",
    "date": "2025-11-14T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Uniformes Colaboradores",
    "customerId": "",
    "customer": "RAFAEL GONDIM BESSA",
    "typeId": 2,
    "type": "Debito",
    "amount": 216.0,
    "accountId": 99,
    "bank": "Santander"
  },
  {
    "id": "tx-0326",
    "date": "2025-11-14T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Uniformes Colaboradores",
    "customerId": "",
    "customer": "RAFAEL GONDIM BESSA",
    "typeId": 2,
    "type": "Debito",
    "amount": 36.0,
    "accountId": 99,
    "bank": "Santander"
  },
  {
    "id": "tx-0327",
    "date": "2025-11-14T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Combustivel",
    "customerId": "",
    "customer": "CATATAU COMERCIO DE COMBU",
    "typeId": 2,
    "type": "Debito",
    "amount": 200.0,
    "accountId": 33,
    "bank": "Santander"
  },
  {
    "id": "tx-0328",
    "date": "2025-11-14T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Pró-labore",
    "customerId": "",
    "customer": "JULIO CESAR RODRIGUES PIR",
    "typeId": 3,
    "type": "Debito",
    "amount": 50.0,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0329",
    "date": "2025-11-17T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Lanches para colaboradores",
    "customerId": "",
    "customer": "TETE SUPERMERCADO E COMER",
    "typeId": 2,
    "type": "Debito",
    "amount": 22.45,
    "accountId": 65,
    "bank": "Santander"
  },
  {
    "id": "tx-0330",
    "date": "2025-11-17T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Locação de Camera",
    "customerId": "",
    "customer": "Adonys Shakaryan Ramires",
    "typeId": 2,
    "type": "Debito",
    "amount": 150.0,
    "accountId": 88,
    "bank": "Santander"
  },
  {
    "id": "tx-0331",
    "date": "2025-11-17T12:00:00",
    "description": "PAGAMENTO CARTAO CREDITO",
    "history": "Pro-labore",
    "customerId": "",
    "customer": "CARTAO MASTER",
    "typeId": 3,
    "type": "Debito",
    "amount": 819.23,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0332",
    "date": "2025-11-17T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Pró-labore",
    "customerId": "",
    "customer": "JULIO CESAR RODRIGUES PIR",
    "typeId": 3,
    "type": "Debito",
    "amount": 700.0,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0333",
    "date": "2025-11-17T12:00:00",
    "description": "PIX RECEBIDO",
    "history": "Transferencias entre contas",
    "customerId": "",
    "customer": "41 655 312 JULIO CESAR RO",
    "typeId": 1,
    "type": "Credito",
    "amount": 1997.0,
    "accountId": 106,
    "bank": "Santander"
  },
  {
    "id": "tx-0334",
    "date": "2025-11-17T12:00:00",
    "description": "COMPRA CARTAO",
    "history": "Lanches para colaboradores",
    "customerId": "",
    "customer": "IDEAL DELICATESSEN",
    "typeId": 2,
    "type": "Debito",
    "amount": 7.09,
    "accountId": 65,
    "bank": "Santander"
  },
  {
    "id": "tx-0335",
    "date": "2025-11-17T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Tripé",
    "customerId": "",
    "customer": "MERCADO PAGO INSTITUICAO",
    "typeId": 2,
    "type": "Debito",
    "amount": 49.44,
    "accountId": 73,
    "bank": "Santander"
  },
  {
    "id": "tx-0336",
    "date": "2025-11-17T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Trieinamento",
    "customerId": "",
    "customer": "MERCADO PAGO INSTITUICAO",
    "typeId": 2,
    "type": "Debito",
    "amount": 149.7,
    "accountId": 98,
    "bank": "Santander"
  },
  {
    "id": "tx-0337",
    "date": "2025-11-17T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Programa de Edição",
    "customerId": "",
    "customer": "MERCADO PAGO INSTITUICAO",
    "typeId": 3,
    "type": "Debito",
    "amount": 95.0,
    "accountId": 25,
    "bank": "Santander"
  },
  {
    "id": "tx-0338",
    "date": "2025-11-17T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Parcela Moto",
    "customerId": "",
    "customer": "MERCADO PAGO INSTITUICAO",
    "typeId": 3,
    "type": "Debito",
    "amount": 200.0,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0339",
    "date": "2025-11-18T12:00:00",
    "description": "COMPRA CARTAO",
    "history": "Placa HYPE",
    "customerId": "",
    "customer": "AliceArtefatosEm",
    "typeId": 2,
    "type": "Debito",
    "amount": 68.0,
    "accountId": 77,
    "bank": "Santander"
  },
  {
    "id": "tx-0340",
    "date": "2025-11-19T12:00:00",
    "description": "COMPRA CARTAO",
    "history": "Combustivel",
    "customerId": "",
    "customer": "POSTO AVENIDA",
    "typeId": 2,
    "type": "Debito",
    "amount": 100.0,
    "accountId": 33,
    "bank": "Santander"
  },
  {
    "id": "tx-0341",
    "date": "2025-11-19T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Lanches para colaboradores",
    "customerId": "",
    "customer": "RONILDO CASSIO DE CAMPOS",
    "typeId": 2,
    "type": "Debito",
    "amount": 15.0,
    "accountId": 65,
    "bank": "Santander"
  },
  {
    "id": "tx-0342",
    "date": "2025-11-19T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Pró-labore",
    "customerId": "",
    "customer": "JULIO CESAR RODRIGUES PIR",
    "typeId": 3,
    "type": "Debito",
    "amount": 300.0,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0343",
    "date": "2025-11-21T12:00:00",
    "description": "PIX RECEBIDO",
    "history": "Esporadico",
    "customerId": "",
    "customer": "KARLA RIBEIRO M PORTELA",
    "typeId": 1,
    "type": "Credito",
    "amount": 2750.0,
    "accountId": 2,
    "bank": "Santander"
  },
  {
    "id": "tx-0344",
    "date": "2025-11-21T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Lanches para colaboradores",
    "customerId": "",
    "customer": "MERCADINHO BOM PRECO",
    "typeId": 2,
    "type": "Debito",
    "amount": 8.5,
    "accountId": 65,
    "bank": "Santander"
  },
  {
    "id": "tx-0345",
    "date": "2025-11-21T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Lanches para colaboradores",
    "customerId": "",
    "customer": "RONILDO CASSIO DE CAMPOS",
    "typeId": 2,
    "type": "Debito",
    "amount": 15.0,
    "accountId": 65,
    "bank": "Santander"
  },
  {
    "id": "tx-0346",
    "date": "2025-11-21T12:00:00",
    "description": "COMPRA CARTAO",
    "history": "Manutenção de Veículo",
    "customerId": "",
    "customer": "AUTO PECAS O BICUDO",
    "typeId": 2,
    "type": "Debito",
    "amount": 15.0,
    "accountId": 70,
    "bank": "Santander"
  },
  {
    "id": "tx-0347",
    "date": "2025-11-21T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Pró-labore",
    "customerId": "",
    "customer": "JULIO CESAR RODRIGUES PIR",
    "typeId": 3,
    "type": "Debito",
    "amount": 50.0,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0348",
    "date": "2025-11-24T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Pró-labore",
    "customerId": "",
    "customer": "JULIO CESAR RODRIGUES PIR",
    "typeId": 3,
    "type": "Debito",
    "amount": 100.0,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0349",
    "date": "2025-11-24T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Pró-labore",
    "customerId": "",
    "customer": "JULIO CESAR RODRIGUES PIR",
    "typeId": 3,
    "type": "Debito",
    "amount": 500.0,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0350",
    "date": "2025-11-25T12:00:00",
    "description": "PIX RECEBIDO",
    "history": "Transferencias entre contas",
    "customerId": "",
    "customer": "41 655 312 JULIO CESAR RO",
    "typeId": 1,
    "type": "Credito",
    "amount": 2497.0,
    "accountId": 106,
    "bank": "Santander"
  },
  {
    "id": "tx-0351",
    "date": "2025-11-25T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Pró-labore",
    "customerId": "",
    "customer": "JULIO CESAR RODRIGUES PIR",
    "typeId": 3,
    "type": "Debito",
    "amount": 100.0,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0352",
    "date": "2025-11-25T12:00:00",
    "description": "COMPRA CARTAO",
    "history": "Combustivel",
    "customerId": "",
    "customer": "POSTO CATATAU",
    "typeId": 2,
    "type": "Debito",
    "amount": 100.0,
    "accountId": 33,
    "bank": "Santander"
  },
  {
    "id": "tx-0353",
    "date": "2025-11-26T12:00:00",
    "description": "COMPRA CARTAO",
    "history": "Lanches para colaboradores",
    "customerId": "",
    "customer": "IDEAL DELICATESSEN",
    "typeId": 2,
    "type": "Debito",
    "amount": 8.3,
    "accountId": 65,
    "bank": "Santander"
  },
  {
    "id": "tx-0354",
    "date": "2025-11-26T12:00:00",
    "description": "COMPRA CARTAO",
    "history": "Lanches para colaboradores",
    "customerId": "",
    "customer": "LIPI SUPERMERCADO",
    "typeId": 2,
    "type": "Debito",
    "amount": 13.99,
    "accountId": 65,
    "bank": "Santander"
  },
  {
    "id": "tx-0355",
    "date": "2025-11-26T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Pró-labore",
    "customerId": "",
    "customer": "JULIO CESAR RODRIGUES PIR",
    "typeId": 3,
    "type": "Debito",
    "amount": 500.0,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0356",
    "date": "2025-11-27T12:00:00",
    "description": "PIX RECEBIDO",
    "history": "Esporadico",
    "customerId": "",
    "customer": "ALBETIZA FARIAS FURTADO",
    "typeId": 1,
    "type": "Credito",
    "amount": 325.0,
    "accountId": 2,
    "bank": "Santander"
  },
  {
    "id": "tx-0357",
    "date": "2025-11-28T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Pró-labore",
    "customerId": "",
    "customer": "JULIO CESAR RODRIGUES PIR",
    "typeId": 3,
    "type": "Debito",
    "amount": 2000.0,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0358",
    "date": "2025-11-28T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Taxa de alteração",
    "customerId": "",
    "customer": "SECRETARIA DA FAZENDA DO",
    "typeId": 2,
    "type": "Debito",
    "amount": 147.85,
    "accountId": 112,
    "bank": "Santander"
  },
  {
    "id": "tx-0359",
    "date": "2025-11-28T12:00:00",
    "description": "PIX RECEBIDO",
    "history": "Mensalidades",
    "customerId": "",
    "customer": "SANTOS E CUNHA LTDA",
    "typeId": 1,
    "type": "Credito",
    "amount": 2300.0,
    "accountId": 3,
    "bank": "Santander"
  },
  {
    "id": "tx-0360",
    "date": "2025-12-01T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Pró-labore",
    "customerId": "",
    "customer": "JULIO CESAR RODRIGUES PIR",
    "typeId": 3,
    "type": "Debito",
    "amount": 200.0,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0361",
    "date": "2025-12-01T12:00:00",
    "description": "COMPRA CARTAO",
    "history": "Pró-labore",
    "customerId": "",
    "customer": "XIMENES ATACAREJO",
    "typeId": 3,
    "type": "Debito",
    "amount": 110.82,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0362",
    "date": "2025-12-01T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Bolsa Estágio",
    "customerId": "",
    "customer": "Saimon Nascimento Silva",
    "typeId": 3,
    "type": "Debito",
    "amount": 966.6,
    "accountId": 18,
    "bank": "Santander"
  },
  {
    "id": "tx-0363",
    "date": "2025-12-01T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Salários",
    "customerId": "",
    "customer": "JOAO VITOR FONTENELE DA S",
    "typeId": 3,
    "type": "Debito",
    "amount": 1650.0,
    "accountId": 18,
    "bank": "Santander"
  },
  {
    "id": "tx-0364",
    "date": "2025-12-01T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Decimo Terceiro João Vitor",
    "customerId": "",
    "customer": "JULIO CESAR RODRIGUES PIR",
    "typeId": 3,
    "type": "Debito",
    "amount": 1500.0,
    "accountId": 28,
    "bank": "Santander"
  },
  {
    "id": "tx-0365",
    "date": "2025-12-01T12:00:00",
    "description": "COMPRA CARTAO",
    "history": "Lanches para colaboradores",
    "customerId": "",
    "customer": "AGB LANCHONETE",
    "typeId": 2,
    "type": "Debito",
    "amount": 14.0,
    "accountId": 65,
    "bank": "Santander"
  },
  {
    "id": "tx-0366",
    "date": "2025-12-01T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Pró-labore",
    "customerId": "",
    "customer": "JULIO CESAR RODRIGUES PIR",
    "typeId": 3,
    "type": "Debito",
    "amount": 100.0,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0367",
    "date": "2025-12-01T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Pró-labore",
    "customerId": "",
    "customer": "JULIO CESAR RODRIGUES PIR",
    "typeId": 3,
    "type": "Debito",
    "amount": 50.0,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0368",
    "date": "2025-12-02T12:00:00",
    "description": "COMPRA CARTAO",
    "history": "Acessório celular da HYPE",
    "customerId": "",
    "customer": "DENNYSCELL UBAJARA",
    "typeId": 2,
    "type": "Debito",
    "amount": 35.0,
    "accountId": 44,
    "bank": "Santander"
  },
  {
    "id": "tx-0369",
    "date": "2025-12-03T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Impostos MEI",
    "customerId": "",
    "customer": "RECEITA FEDERAL",
    "typeId": 2,
    "type": "Debito",
    "amount": 80.9,
    "accountId": 89,
    "bank": "Santander"
  },
  {
    "id": "tx-0370",
    "date": "2025-12-03T12:00:00",
    "description": "PIX RECEBIDO",
    "history": "Esporádico",
    "customerId": "",
    "customer": "R F M C LTDA",
    "typeId": 1,
    "type": "Credito",
    "amount": 8000.0,
    "accountId": 2,
    "bank": "Santander"
  },
  {
    "id": "tx-0371",
    "date": "2025-12-03T12:00:00",
    "description": "COMPRA CARTAO",
    "history": "Acessório celular da HYPE",
    "customerId": "",
    "customer": "DENNYSCELL UBAJARA",
    "typeId": 2,
    "type": "Debito",
    "amount": 30.0,
    "accountId": 44,
    "bank": "Santander"
  },
  {
    "id": "tx-0372",
    "date": "2025-12-04T12:00:00",
    "description": "PIX RECEBIDO",
    "history": "Mensalidades",
    "customerId": "",
    "customer": "CASA DE CECÍLIA",
    "typeId": 1,
    "type": "Credito",
    "amount": 3000.0,
    "accountId": 3,
    "bank": "Santander"
  },
  {
    "id": "tx-0373",
    "date": "2025-12-04T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Pró-labore",
    "customerId": "",
    "customer": "JULIO CESAR RODRIGUES PIR",
    "typeId": 3,
    "type": "Debito",
    "amount": 2000.0,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0374",
    "date": "2025-12-09T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Pró-labore",
    "customerId": "",
    "customer": "JULIO CESAR RODRIGUES PIR",
    "typeId": 3,
    "type": "Debito",
    "amount": 2066.0,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0375",
    "date": "2025-12-09T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Serviços Terceirizados",
    "customerId": "",
    "customer": "EURISBERTO FERREIRA AGUIA",
    "typeId": 2,
    "type": "Debito",
    "amount": 350.0,
    "accountId": 88,
    "bank": "Santander"
  },
  {
    "id": "tx-0376",
    "date": "2025-12-10T12:00:00",
    "description": "TARIFA MENSALIDADE",
    "history": "Manutenção da Conta",
    "customerId": "",
    "customer": "PACOTE SERVICOS NOVEMBRO / 2025",
    "typeId": 2,
    "type": "Debito",
    "amount": 229.0,
    "accountId": 67,
    "bank": "Santander"
  },
  {
    "id": "tx-0377",
    "date": "2025-12-10T12:00:00",
    "description": "COMPRA CARTAO",
    "history": "Lanches para colaboradores",
    "customerId": "",
    "customer": "IDEAL DELICATESSEN",
    "typeId": 2,
    "type": "Debito",
    "amount": 11.0,
    "accountId": 65,
    "bank": "Santander"
  },
  {
    "id": "tx-0378",
    "date": "2025-12-11T12:00:00",
    "description": "COMPRA CARTAO",
    "history": "Lanches para colaboradores",
    "customerId": "",
    "customer": "MP .MBBOLOS",
    "typeId": 2,
    "type": "Debito",
    "amount": 12.0,
    "accountId": 65,
    "bank": "Santander"
  },
  {
    "id": "tx-0379",
    "date": "2025-12-12T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Pró-labore",
    "customerId": "",
    "customer": "JULIO CESAR RODRIGUES PIR",
    "typeId": 3,
    "type": "Debito",
    "amount": 100.0,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0380",
    "date": "2025-12-15T12:00:00",
    "description": "COMPRA CARTAO",
    "history": "Combustível",
    "customerId": "",
    "customer": "POSTO COLIBRI EMPRE",
    "typeId": 2,
    "type": "Debito",
    "amount": 100.0,
    "accountId": 33,
    "bank": "Santander"
  },
  {
    "id": "tx-0381",
    "date": "2025-12-15T12:00:00",
    "description": "PAGAMENTO CARTAO CREDITO",
    "history": "Pró-labore",
    "customerId": "",
    "customer": "SANTANDER",
    "typeId": 3,
    "type": "Debito",
    "amount": 825.83,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0382",
    "date": "2025-12-15T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Tripé",
    "customerId": "",
    "customer": "MERCADO PAGO INSTITUICAO",
    "typeId": 2,
    "type": "Debito",
    "amount": 49.44,
    "accountId": 73,
    "bank": "Santander"
  },
  {
    "id": "tx-0383",
    "date": "2025-12-15T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Treinamento",
    "customerId": "",
    "customer": "MERCADO PAGO INSTITUICAO",
    "typeId": 2,
    "type": "Debito",
    "amount": 149.7,
    "accountId": 98,
    "bank": "Santander"
  },
  {
    "id": "tx-0384",
    "date": "2025-12-15T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Programa de Edição",
    "customerId": "",
    "customer": "MERCADO PAGO INSTITUICAO",
    "typeId": 3,
    "type": "Debito",
    "amount": 95.0,
    "accountId": 25,
    "bank": "Santander"
  },
  {
    "id": "tx-0385",
    "date": "2025-12-15T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Parcela Moto",
    "customerId": "",
    "customer": "MERCADO PAGO INSTITUICAO",
    "typeId": 3,
    "type": "Debito",
    "amount": 200.0,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0386",
    "date": "2025-12-15T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Internet",
    "customerId": "",
    "customer": "BRISANET",
    "typeId": 2,
    "type": "Debito",
    "amount": 86.66,
    "accountId": 57,
    "bank": "Santander"
  },
  {
    "id": "tx-0387",
    "date": "2025-12-15T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Pró-labore",
    "customerId": "",
    "customer": "JULIO CESAR RODRIGUES PIR",
    "typeId": 3,
    "type": "Debito",
    "amount": 1700.0,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0388",
    "date": "2025-12-15T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Transferencias entre contas",
    "customerId": "",
    "customer": "41655312 JULIO CESAR RODR",
    "typeId": 2,
    "type": "Debito",
    "amount": 5501.83,
    "accountId": 104,
    "bank": "Santander"
  },
  {
    "id": "tx-0389",
    "date": "2025-12-15T12:00:00",
    "description": "PIX RECEBIDO",
    "history": "Esporádico",
    "customerId": "",
    "customer": "ALBETIZA FARIAS FURTADO",
    "typeId": 1,
    "type": "Credito",
    "amount": 300.0,
    "accountId": 2,
    "bank": "Santander"
  },
  {
    "id": "tx-0390",
    "date": "2025-12-16T12:00:00",
    "description": "PIX ENVIADO",
    "history": "Transferencias entre contas",
    "customerId": "",
    "customer": "41655312 JULIO CESAR RODR",
    "typeId": 2,
    "type": "Debito",
    "amount": 300.0,
    "accountId": 104,
    "bank": "Santander"
  },
  {
    "id": "tx-0391",
    "date": "2026-01-21T12:00:00",
    "description": "PIX RECEBIDO",
    "history": "Transferencias entre contas",
    "customerId": "",
    "customer": "HYPE PRODUTORA",
    "typeId": 1,
    "type": "Credito",
    "amount": 960.0,
    "accountId": 106,
    "bank": "Santander"
  },
  {
    "id": "tx-0392",
    "date": "2026-01-21T12:00:00",
    "description": "PAGAMENTO CARTAO CREDITO",
    "history": "Pró-labore",
    "customerId": "",
    "customer": "CARTAO MASTER",
    "typeId": 3,
    "type": "Debito",
    "amount": 953.01,
    "accountId": 103,
    "bank": "Santander"
  },
  {
    "id": "tx-0393",
    "date": "2026-01-23T12:00:00",
    "description": "TARIFA MENSALIDADE",
    "history": "Manutenção da Conta",
    "customerId": "",
    "customer": "",
    "typeId": 2,
    "type": "Debito",
    "amount": 239.0,
    "accountId": 67,
    "bank": "Santander"
  }
];

export const mockMonthly: { 
  month: MockMonthlyType[],
  averageOrderValueFixed: number,
  averageOrderValueEvent: number 
} = createMockMonthly(mockTransactions);

export const chartAccounts: ChartAccountsType[] = [
  { id: 1, type: "Receitas", subcategories: [
    { accountId: 2, categories: "Projetos Avulsos", subordinate: "Receitas Variáveis" },
    { accountId: 3, categories: "Contratos Mensais", subordinate: "Receita Fixo" },
    { accountId: 4, categories: "Resgate de Aplicação", subordinate: "Retorno de Capital" },
    { accountId: 5, categories: "Juros e Multas", subordinate: "Outras Receitas" },
    { accountId: 111, categories: "Estornos", subordinate: "Outras Receitas" },
    { accountId: 106, categories: "Transferências", subordinate: "" }
  ] },
  { id: 2, type: "Despesas", subcategories: [
    { accountId: 30, categories: "Despesas Administrativas", subordinate: "" },
    { accountId: 6, categories:	"Agua", subordinate: "Despesas Administrativas" },
    { accountId: 7, categories: "Alugueis", subordinate: "Despesas Administrativas" },
    { accountId: 21, categories: "Compras Esporádicas", subordinate: "Despesas Administrativas" },
    { accountId: 23, categories: "Construções/Reformas", subordinate: "Despesas Administrativas" },
    { accountId: 24, categories: "Consultorias", subordinate: "Despesas Administrativas" },
    { accountId: 31, categories: "Despesas Com Clientes", subordinate: "Despesas Administrativas" },
    { accountId: 33, categories: "Despesas Com Veiculos", subordinate: "Despesas Administrativas" },
    { accountId: 34, categories: "Despesas Com Viagens e Hospedagens", subordinate: "Despesas Administrativas" },
    { accountId: 43, categories: "Energia", subordinate: "Despesas Administrativas" },
    { accountId: 44, categories: "Equip. Uso e Consumo", subordinate: "Despesas Administrativas" },
    { accountId: 57, categories: "Internet", subordinate: "Despesas Administrativas" },
    { accountId: 60, categories: "IPVA", subordinate: "Despesas Administrativas" },
    { accountId: 63, categories: "Juros e Multas", subordinate: "Despesas Administrativas" },
    { accountId: 68, categories: "Manutenção de Equipamentos", subordinate: "Despesas Administrativas" },
    { accountId: 69, categories: "Manutenção de Informatica", subordinate: "Despesas Administrativas" },
    { accountId: 70, categories: "Manutenção de Veiculos", subordinate: "Despesas Administrativas" },
    { accountId: 71, categories: "Manutenção Interna (máquinas, Luzes...)", subordinate: "Despesas Administrativas" },
    { accountId: 72, categories: "Manutenção Predial", subordinate: "Despesas Administrativas" },
    { accountId: 73, categories: "Maquinas e Equipamentos", subordinate: "Despesas Administrativas" },
    { accountId: 74, categories: "Materiais de Higiene e Limpeza", subordinate: "Despesas Administrativas" },
    { accountId: 75, categories: "Material de Escritório", subordinate: "Despesas Administrativas" },
    { accountId: 76, categories: "Material Gráfico", subordinate: "Despesas Administrativas" },
    { accountId: 77, categories: "Moveis e Utensilios", subordinate: "Despesas Administrativas" },
    { accountId: 86, categories: "Seguros", subordinate: "Despesas Administrativas" },
    { accountId: 88, categories: "Serviços Terceirizados", subordinate: "Despesas Administrativas" },
    { accountId: 90, categories: "Sistemas Contratados", subordinate: "Despesas Administrativas" },
    { accountId: 95, categories: "Telefonia e Planos", subordinate: "Despesas Administrativas" },
    { accountId: 96, categories: "Terrenos e Imoveis", subordinate: "Despesas Administrativas" },
    { accountId: 98, categories: "Treinamentos e Cursos", subordinate: "Despesas Administrativas" },
    { accountId: 112, categories: "Taxas e Contribuições", subordinate: "Despesas Administrativas" },
    { accountId: 8,	categories: "Aplicações", subordinate: "Investimentos" },
    { accountId: 9, categories:	"Ativo Imobilizado", subordinate: "" },
    { accountId: 1, categories: "Adiantamento de Salário", subordinate:	"Despesas Com Colaboradores" },
    { accountId: 10, categories: "Beneficios Não Obrigatorios", subordinate: "Despesas Com Colaboradores" },
    { accountId: 12, categories: "Bonus por Metas", subordinate: "Despesas Com Colaboradores" },
    { accountId: 13, categories: "Brindes", subordinate: "Eventos e Confraternizações" },
    { accountId: 14, categories: "Comissão dos Colaboradores", subordinate: "Despesas Com Colaboradores" },
    { accountId: 17, categories: "Bolsa Estágio", subordinate: "Despesas Com Colaboradores" },
    { accountId: 19, categories: "Folha de Pagamento", subordinate: "Despesas Com Colaboradores" },
    { accountId: 20, categories: "Compras", subordinate: "" },
    { accountId: 22, categories: "Confraternizações", subordinate: "Eventos e Confraternizações" },
    { accountId: 29, categories: "Decimo Terceiro Salario", subordinate: "Despesas Com Colaboradores" },
    { accountId: 32, categories: "Despesas Com Colaboradores", subordinate: "" },
    { accountId: 35, categories: "Despesas de Estágio", subordinate: "Despesas Com Colaboradores" },
    { accountId: 36, categories: "Despesas Eventuais", subordinate: "" },
    { accountId: 38, categories: "Devoluções", subordinate: "Outras Despesas" },
    { accountId: 39, categories: "Emprestimos", subordinate: "Emprestimos e Ficiamentos" },
    { accountId: 40, categories: "Emprestimos e Ficiamentos", subordinate: "" },
    { accountId: 41, categories: "Emprestimos Internos", subordinate: "Outras Despesas" },
    { accountId: 42, categories: "Encargos Bancários", subordinate: "" },
    { accountId: 45, categories: "Eventos e Confraternizações", subordinate: "" },
    { accountId: 48, categories: "Férias", subordinate: "Despesas Com Colaboradores" },
    { accountId: 52, categories: "Impostos", subordinate: "Impostos Mensais e Trimestrais" },
    { accountId: 53, categories: "Impostos Mensais e Trimestrais", subordinate: "" },
    { accountId: 56, categories: "Integralização de Capital", subordinate: "Investimentos" },
    { accountId: 58, categories: "Investimentos", subordinate: "" },
    { accountId: 59, categories: "IPTU", subordinate: "Impostos Mensais e Trimestrais" },
    { accountId: 61, categories: "Jantares/Almoços Em Confraternizações", subordinate: "Eventos e Confraternizações" },
    { accountId: 62, categories: "Juros de Emprestimos", subordinate: "Emprestimos e Ficiamentos" },
    { accountId: 64, categories: "Juros Utiliz Ch Especial", subordinate: "Encargos Bancários" },
    { accountId: 65, categories: "Lanches e Refeições", subordinate: "Despesas Com Colaboradores" },
    { accountId: 66, categories: "Lembranças e Troféus", subordinate: "Eventos e Confraternizações" },
    { accountId: 67, categories: "Manutençao da Conta", subordinate: "Encargos Bancários" },
    { accountId: 79, categories: "Outras Despesas", subordinate: "" },
    { accountId: 80, categories: "Outras Despesas", subordinate: "Despesas Eventuais" },
    { accountId: 81, categories: "Outras Tarifas", subordinate: "Encargos Bancários" },
    { accountId: 84, categories: "Rescisões", subordinate: "Despesas Com Colaboradores" },
    { accountId: 87, categories: "Serviços Extraordinários", subordinate: "Eventos e Confraternizações" },
    { accountId: 89, categories: "Simples Nacional", subordinate: "Impostos Mensais e Trimestrais" },
    { accountId: 91, categories: "Tarifas Bancárias", subordinate: "Encargos Bancários" },
    { accountId: 92, categories: "Tarifas de Boletos", subordinate: "Encargos Bancários" },
    { accountId: 93, categories: "Tarifas Pix", subordinate: "Encargos Bancários" },
    { accountId: 94, categories: "Tarifas Utiliz. Cartão", subordinate: "Encargos Bancários" },
    { accountId: 97, categories: "Transferencias", subordinate: "" },
    { accountId: 99, categories: "Uniformes e Crachás", subordinate: "Despesas Com Colaboradores" },
    { accountId: 100, categories: "Vale Transportes", subordinate: "Despesas Com Colaboradores" },
    { accountId: 102, categories: "Copa e Cozinha", subordinate: "Despesas com Colaboradores" },
    { accountId: 104, categories: "Transferências", subordinate: "" }
  ] },
  { id: 3, type: "Custos", subcategories: [
    { accountId: 26, categories: "Custos", subordinate: "" },
    { accountId: 11, categories: "Benefícios", subordinate: "Custo" },
    { accountId: 15, categories: "Comissões", subordinate: "Custo" },
    { accountId: 16, categories: "Bolsa Estágio", subordinate: "Custo" },
    { accountId: 18, categories: "Folha de Pagamento", subordinate: "Custo" },
    { accountId: 25, categories: "Custo com Sistemas", subordinate: "Custo" },
    { accountId: 28, categories: "Décimo Terceiro", subordinate: "Custo" },
    { accountId: 101, categories: "Entregas ao Cliente", subordinate: "Custo" },
    { accountId: 46, categories: "Exames ASO", subordinate: "Custo" },
    { accountId: 47, categories: "Férias", subordinate: "Custo" },
    { accountId: 49, categories: "FGTS", subordinate: "Custo" },
    { accountId: 50, categories: "FGTS Rescisório", subordinate: "Custo" },
    { accountId: 51, categories: "Horas Extras", subordinate: "Custo" },
    { accountId: 54, categories: "INSS", subordinate: "Custo" },
    { accountId: 55, categories: "INSS/IRRF", subordinate: "Custo" },
    { accountId: 85, categories: "Rescisões", subordinate: "Custo" },
    { accountId: 103, categories: "Pró-labore", subordinate: "Custo" },
  ] },
];
